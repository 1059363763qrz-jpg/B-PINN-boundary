...
RUN_QCAL_ALLTHETA_MC2500_EVAL_ONLY = True
...
ALL_THETA_COMPARISON_DIR = "training_results_theta_independent_v7style_no_pcc_branch_limit_data1000_mc60_qcal_only_comparison_mc2500"
...
def eval_all_theta_multiseed_v7style(case,data_dict,model_map):
    ...
    for seed in ALL_THETA_EVAL_SEEDS:
        ...
        for j in ALL_THETA_LIST:
            ...
            print(f"[mc2500-eval] seed={seed:03d} theta={j:02d} start", flush=True)
            ...
            rows.append(m)
            done_jobs += 1
            print(f"[mc2500-eval] seed={seed:03d} theta={j:02d} done, ARMS={m['external_cdf_arms']:.4f}, opf_per_sample_ms={m['opf_per_sample_ms']:.3f}", flush=True)
            print(f"[mc2500-eval] progress {done_jobs}/{total_jobs}", flush=True)
...
def main_v7_4_2_qcal_alltheta_mc2500_eval_only():
    print('[v7.4.2-qcal-alltheta-mc2500-eval-only] start',flush=True)
    ...
    ALL_THETA_EVAL_SEEDS=[1,2,3]; ALL_THETA_EVAL_MC=2500
    ...
    print(f'[v7.4.2] missing theta list = {missing}', flush=True)
    if missing:
        raise RuntimeError(f'Missing qcal model_map entries for theta={missing}')
    print('[v7.4.2] start all-theta external MC2500 evaluation: 12 theta × 3 seeds × 2500 MC', flush=True)
    eval_all_theta_multiseed_v7style(case,data_dict,model_map)
    write_qcal_alltheta_external_summary_aliases()
    summary_path=Path(ALL_THETA_COMPARISON_DIR)/'all_theta_multiseed_external_by_theta_seed.csv'
    if summary_path.exists():
        df=pd.read_csv(summary_path)
        print(f"[v7.4.2] mean ARMS = {float(df['external_cdf_arms'].mean()):.4f}", flush=True)
        print(f"[v7.4.2] median ARMS = {float(df['external_cdf_arms'].median()):.4f}", flush=True)
        print(f"[v7.4.2] max ARMS = {float(df['external_cdf_arms'].max()):.4f}", flush=True)
        worst=df.sort_values('external_cdf_arms',ascending=False).head(10)[['theta_idx','seed','external_cdf_arms','q50_error']]
        print('[v7.4.2] worst 10 theta/seed rows:', flush=True)
        print(worst.to_string(index=False), flush=True)
    print(f'[v7.4.2] MC2500 output dir = {ALL_THETA_COMPARISON_DIR}', flush=True)
    print('[v7.4.2-qcal-alltheta-mc2500-eval-only] done', flush=True)

if __name__ == '__main__':
    main_v7_4_2_qcal_alltheta_mc2500_eval_only()
