...
def train_bayes_flex_gmm2(case,XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG):
    ...
    for ep in range(EPOCHS):
        ...
        perm=rng.permutation(n); beta=BETA_KL_MAX*min(1.0,(ep+1)/KL_WARMUP_EPOCHS); acc=np.zeros(15)
        for b in range(nb):
            ...
            kl=net.kl_divergence(); loss=nll+cur_lam_boundary*bsup+cur_lam_dispatch*dsup+cur_lam_phys*phys+LAM_SUPPORT_CONSIST*scons+cur_lam_hq*hq+cur_lam_hcdf*hcdf+cur_lam_loc*h_loc_cal+cur_lam_mean*h_mean_cal+cur_lam_scale*h_scale_cal+cur_lam_std*h_std_cal+cur_lam_tail*h_tail_cal+beta*kl/n
            opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(net.parameters(),5.0); opt.step()
            acc += np.array([float(loss.detach().cpu()),float(nll.detach().cpu()),float(bsup.detach().cpu()),float(p0q0_sup.detach().cpu()),float(t_sup.detach().cpu()),float(dsup.detach().cpu()),float(phys.detach().cpu()),float(scons.detach().cpu()),float(hq.detach().cpu()),float(hcdf.detach().cpu()),float(h_loc_cal.detach().cpu()),float(h_mean_cal.detach().cpu()),float(h_scale_cal.detach().cpu()),float(h_std_cal.detach().cpu()),float(h_tail_cal.detach().cpu())])
        if ep==0 or (ep+1)%50==0 or ep+1==EPOCHS:
            ...
            if reweight_enabled and ((ep+1)%THETA_REWEIGHT_UPDATE_EVERY==0):
                w=(th_cdf+1e-9)**THETA_REWEIGHT_POWER; w=w/(w.mean()+1e-9); w=np.clip(w,THETA_REWEIGHT_MIN,THETA_REWEIGHT_MAX); theta_sampling_weights=w; combined_theta_weights=theta_problem_weights*theta_sampling_weights; combined_theta_weights/=combined_theta_weights.mean()+1e-9; print(f"[calib-theta-weights] updated combined weights = {np.round(combined_theta_weights,3).tolist()}")
            vscore=float(hcdfv.item())+float(hqv.item())+float(val_loc_cal.item())+float(val_scale_cal.item())+0.5*float(val_tail_cal.item())+0.2*float(bsv.item())+0.2*float(phv.item()); print(f"[best-score] vscore={vscore:.6f} best_score={best_score:.6f}")
    ...
    eval_cached_validation_cdf_arms(net,norm,XMU_va,THETA_FEAT,THETA_LIST,YH_va,save_prefix="cached_val_cdf_arms_v6")
    train_info={"XMU_val":XMU_va,"YH_val":YH_va,"THETA_FEAT":THETA_FEAT,"theta_list":THETA_LIST,"cached_val_arms_csv":"cached_val_cdf_arms_v6.csv"}
    return net,norm,train_info

def eval_cached_validation_cdf_arms(net,norm,XMU_val,THETA_FEAT,theta_list,YH_val,save_prefix="cached_val_cdf_arms_v6",n_grid=300,n_posterior_samples=0):
    ...
    for j,th in enumerate(theta_list):
        ...
        rows.append({"theta_idx":j,"theta":float(th),"arms_pct":float(np.mean(arms_l)),"ks_stat":float(np.mean(ks_l)),"q50_err":float(np.mean(q50_l)),"span95_err":float(np.mean(sp_l)),"std_ratio":float(np.mean(sr_l))})
    with open(f"{save_prefix}.csv","w",newline="",encoding="utf-8") as f:
        w=csv.DictWriter(f,fieldnames=["theta_idx","theta","arms_pct","ks_stat","q50_err","span95_err","std_ratio"]); w.writeheader(); w.writerows(rows)
    vals=np.array([r['arms_pct'] for r in rows]); worst=np.argsort(-vals)[:3].tolist(); print(f"[cached-val-arms] mean={vals.mean():.4f}"); print(f"[cached-val-arms] median={np.median(vals):.4f}"); print(f"[cached-val-arms] max={vals.max():.4f}"); print(f"[cached-val-arms] worst3={worst}")
    colors=['#6b7280']*len(rows)
    for wi in worst: colors[wi]='#dc2626'
    plt.figure(figsize=(9,4.2),dpi=220); plt.bar([r['theta_idx'] for r in rows],[r['arms_pct'] for r in rows],color=colors); plt.axhline(vals.mean(),color='#1d4ed8',ls='--',lw=1.5,label='mean ARMS'); plt.xlabel('theta_idx'); plt.ylabel('ARMS (%)'); plt.title('Cached validation CDF ARMS of support-function distribution'); plt.legend(); plt.tight_layout(); plt.savefig(f"{save_prefix}_bar_v6.png",dpi=260); plt.close()
    return rows
