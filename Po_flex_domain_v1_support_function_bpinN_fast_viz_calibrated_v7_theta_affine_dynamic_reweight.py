...
LOCATION_THETA_INIT = [2, 3, 6, 7, 9, 10]
SCALE_THETA_INIT = [0, 1, 5]
SHAPE_THETA_INIT = [8, 11]
...
def train_bayes_flex_gmm2(case,XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG):
    ...
    for ep in range(EPOCHS):
        ...
        if stage=="A": cur_q_every,cur_q_scen,cur_q_theta,cur_lam_hq,cur_lam_hcdf,cur_lr,cur_lam_loc,cur_lam_mean,cur_lam_scale,cur_lam_std,cur_lam_tail,cur_lam_boundary,cur_lam_dispatch,cur_lam_phys=5,8,4,0.03,0.00,LR,LAM_LOC_CAL_A,LAM_MEAN_CAL_A,LAM_SCALE_CAL_A,LAM_STD_CAL_A,LAM_TAIL_CAL_A,LAM_BOUNDARY_SUP,LAM_DISPATCH_SUP,LAM_PHYS_FLEX
        elif stage=="B": cur_q_every,cur_q_scen,cur_q_theta,cur_lam_hq,cur_lam_hcdf,cur_lr,cur_lam_loc,cur_lam_mean,cur_lam_scale,cur_lam_std,cur_lam_tail,cur_lam_boundary,cur_lam_dispatch,cur_lam_phys=1,32,None,0.15,0.15,LR*0.3,LAM_LOC_CAL_B,LAM_MEAN_CAL_B,LAM_SCALE_CAL_B,LAM_STD_CAL_B,LAM_TAIL_CAL_B,LAM_BOUNDARY_SUP,LAM_DISPATCH_SUP,LAM_PHYS_FLEX
        elif stage=="C": cur_q_every,cur_q_scen,cur_q_theta,cur_lam_hq,cur_lam_hcdf,cur_lr,cur_lam_loc,cur_lam_mean,cur_lam_scale,cur_lam_std,cur_lam_tail,cur_lam_boundary,cur_lam_dispatch,cur_lam_phys=1,min(48,max(1,XMU_tr.shape[0])),None,0.20,0.25,LR*0.15,LAM_LOC_CAL_C,LAM_MEAN_CAL_C,LAM_SCALE_CAL_C,LAM_STD_CAL_C,LAM_TAIL_CAL_C,LAM_BOUNDARY_SUP,LAM_DISPATCH_SUP,LAM_PHYS_FLEX
        else: cur_q_every,cur_q_scen,cur_q_theta,cur_lam_hq,cur_lam_hcdf,cur_lr,cur_lam_loc,cur_lam_mean,cur_lam_scale,cur_lam_std,cur_lam_tail,cur_lam_boundary,cur_lam_dispatch,cur_lam_phys=1,min(64,max(1,XMU_tr.shape[0])),None,0.20,0.30,LR*0.08,LAM_LOC_CAL_D,LAM_MEAN_CAL_D,LAM_SCALE_CAL_D,LAM_STD_CAL_D,LAM_TAIL_CAL_D,0.4*LAM_BOUNDARY_SUP,0.4*LAM_DISPATCH_SUP,0.4*LAM_PHYS_FLEX
        reweight_enabled=USE_THETA_REWEIGHT and (ep+1)>=THETA_REWEIGHT_START_EPOCH and stage in ["C","D"]
        combined_theta_weights=theta_problem_weights*theta_sampling_weights; combined_theta_weights/=combined_theta_weights.mean()+1e-9
    ...
    eval_cached_validation_cdf_arms(net,norm,XMU_va,THETA_FEAT,THETA_LIST,YH_va,save_prefix="cached_val_cdf_arms_v7")
    train_info={"XMU_val":XMU_va,"YH_val":YH_va,"THETA_FEAT":THETA_FEAT,"theta_list":THETA_LIST,"cached_val_arms_csv":"cached_val_cdf_arms_v7.csv"}
...
def eval_cached_validation_cdf_arms(net,norm,XMU_val,THETA_FEAT,theta_list,YH_val,save_prefix="cached_val_cdf_arms_v7",n_grid=300,n_posterior_samples=0):
