...
LOCATION_THETA_INIT = [6, 7, 5, 1, 2, 10]
SCALE_THETA_INIT = [8, 9, 1]
SHAPE_THETA_INIT = [0, 4]
LOC_THETA_WEIGHT = 2.5
SCALE_THETA_WEIGHT = 2.0
SHAPE_THETA_WEIGHT = 1.5
DEFAULT_THETA_WEIGHT = 1.0
EXTRA_HARD_THETA_INIT = [6]
EXTRA_HARD_THETA_WEIGHT = 4.0
USE_THETA_WISE_CALIB_NORMALIZATION = True
THETA_STD_FLOOR = 1e-3
USE_DENSE_QUANTILE_CALIBRATION = True
...
DATASET_CACHE_MODE = "load_only"  # "auto" | "rebuild" | "load_only"
...
def compute_h_loc_scale_calibration_loss(net,XMU_scen,THETA_FEAT,QH_CAL_EMP,H_MEAN_EMP,H_STD_EMP,x_mu_mean,x_mu_std,h_mean,h_std,n_scenarios_sample=32,n_thetas_sample=None,sample=True,rng=None,theta_sampling_weights=None,theta_problem_weights=None,taus=CALIB_TAUS,return_parts=False):
    ...
    _,w,mu,s=net.forward_gmm(xmu_t,th_t,sample=sample)
    q_emp=QH_CAL_EMP[sf,tf,:]; q_emp_n=(q_emp-h_mean)/(h_std+1e-9)
    m_emp=((H_MEAN_EMP[sf,tf]).reshape(-1,1)-h_mean)/(h_std+1e-9); sd_emp=((H_STD_EMP[sf,tf]).reshape(-1,1)/(h_std+1e-9))
    q_emp_t=torch.as_tensor(q_emp_n,dtype=torch.float32,device=DEVICE)
    m_emp_t=torch.as_tensor(m_emp,dtype=torch.float32,device=DEVICE)
    sd_emp_t=torch.as_tensor(sd_emp,dtype=torch.float32,device=DEVICE)
    q_pred=gmm_quantile_torch(w,mu,s,taus)
...
def compute_h_dense_quantile_calibration_loss(net,XMU_scen,THETA_FEAT,QH_DENSE_EMP,x_mu_mean,x_mu_std,h_mean,h_std,h_theta_mean=None,h_theta_std=None,n_scenarios_sample=32,n_thetas_sample=None,sample=False,rng=None,theta_sampling_weights=None,theta_problem_weights=None,taus=DENSE_Q_TAUS,use_theta_wise_norm=USE_THETA_WISE_CALIB_NORMALIZATION):
    ...
    if use_theta_wise_norm and h_theta_mean is not None and h_theta_std is not None:
        m=h_theta_mean[tf].reshape(-1,1); st=h_theta_std[tf].reshape(-1,1)
        mu=(torch.as_tensor((h_mean+h_std*mu.detach().cpu().numpy()-m)/st,dtype=torch.float32,device=DEVICE))
        s=(torch.as_tensor((h_std*s.detach().cpu().numpy()/st),dtype=torch.float32,device=DEVICE))
        q_emp=(q_emp-m)/st
    else:
...
def train_bayes_flex_gmm2(case,XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG):
    ...
    print(f"[calib] QH_CAL_EMP_TR shape={QH_CAL_EMP_TR.shape}"); print(f"[calib] H_MEAN_EMP_TR shape={H_MEAN_EMP_TR.shape}"); print(f"[calib] H_STD_EMP_TR shape={H_STD_EMP_TR.shape}")
    print("[calib] location/scale calibration uses deterministic posterior mean: sample=False")
    ...
    best_state=None; best_score=np.inf; patience=25; no_improve=0; T=THETA_FEAT.shape[0]; theta_sampling_weights=np.ones(T,dtype=float)
    theta_problem_weights=np.ones(T,dtype=float)*DEFAULT_THETA_WEIGHT
    if USE_STATIC_ERROR_TYPE_WEIGHTS:
        ...
        theta_problem_weights=theta_problem_weights/(theta_problem_weights.mean()+1e-9)
    else: theta_problem_weights=np.ones(T,dtype=float)
    ...
    val_arms_mean_recent=100.0
    val_arms_max_recent=100.0
    print(f"[calib-theta-weights] static problem weights = {np.round(theta_problem_weights,3).tolist()}")
    print(f"[calib-theta-weights] initial dynamic weights = {np.round(theta_sampling_weights,3).tolist()}")
    print(f"[calib-theta-weights] initial combined weights = {np.round(combined_theta_weights,3).tolist()}")
    val_arms_mean_recent=100.0
    val_arms_max_recent=100.0
    prev_stage=None
    for ep in range(EPOCHS):
        ...
        if stage=="A": cur_q_every,cur_q_scen,cur_q_theta,cur_lam_hq,cur_lam_hcdf,cur_lr,cur_lam_loc,cur_lam_mean,cur_lam_scale,cur_lam_std,cur_lam_tail,cur_lam_dense_q,cur_lam_boundary,cur_lam_dispatch,cur_lam_phys=5,8,4,0.03,0.00,LR,LAM_LOC_CAL_A,LAM_MEAN_CAL_A,LAM_SCALE_CAL_A,LAM_STD_CAL_A,LAM_TAIL_CAL_A,LAM_DENSE_Q_CAL_A,LAM_BOUNDARY_SUP,LAM_DISPATCH_SUP,LAM_PHYS_FLEX
        elif stage=="B": cur_q_every,cur_q_scen,cur_q_theta,cur_lam_hq,cur_lam_hcdf,cur_lr,cur_lam_loc,cur_lam_mean,cur_lam_scale,cur_lam_std,cur_lam_tail,cur_lam_dense_q,cur_lam_boundary,cur_lam_dispatch,cur_lam_phys=1,32,None,0.15,0.15,LR*0.3,LAM_LOC_CAL_B,LAM_MEAN_CAL_B,LAM_SCALE_CAL_B,LAM_STD_CAL_B,LAM_TAIL_CAL_B,LAM_DENSE_Q_CAL_B,LAM_BOUNDARY_SUP,LAM_DISPATCH_SUP,LAM_PHYS_FLEX
        elif stage=="C": cur_q_every,cur_q_scen,cur_q_theta,cur_lam_hq,cur_lam_hcdf,cur_lr,cur_lam_loc,cur_lam_mean,cur_lam_scale,cur_lam_std,cur_lam_tail,cur_lam_dense_q,cur_lam_boundary,cur_lam_dispatch,cur_lam_phys=1,min(48,max(1,XMU_tr.shape[0])),None,0.20,0.25,LR*0.15,LAM_LOC_CAL_C,LAM_MEAN_CAL_C,LAM_SCALE_CAL_C,LAM_STD_CAL_C,LAM_TAIL_CAL_C,LAM_DENSE_Q_CAL_C,LAM_BOUNDARY_SUP,LAM_DISPATCH_SUP,LAM_PHYS_FLEX
        elif stage=="D": cur_q_every,cur_q_scen,cur_q_theta,cur_lam_hq,cur_lam_hcdf,cur_lr,cur_lam_loc,cur_lam_mean,cur_lam_scale,cur_lam_std,cur_lam_tail,cur_lam_dense_q,cur_lam_boundary,cur_lam_dispatch,cur_lam_phys=1,min(64,max(1,XMU_tr.shape[0])),None,0.20,0.30,LR*0.08,LAM_LOC_CAL_D,LAM_MEAN_CAL_D,LAM_SCALE_CAL_D,LAM_STD_CAL_D,LAM_TAIL_CAL_D,LAM_DENSE_Q_CAL_D,0.4*LAM_BOUNDARY_SUP,0.4*LAM_DISPATCH_SUP,0.4*LAM_PHYS_FLEX
        else: cur_q_every,cur_q_scen,cur_q_theta,cur_lam_hq,cur_lam_hcdf,cur_lr,cur_lam_loc,cur_lam_mean,cur_lam_scale,cur_lam_std,cur_lam_tail,cur_lam_dense_q,cur_lam_boundary,cur_lam_dispatch,cur_lam_phys=1,min(64,max(1,XMU_tr.shape[0])),None,0.0,0.0,LR*0.05,LAM_LOC_CAL_E,LAM_MEAN_CAL_E,LAM_SCALE_CAL_E,LAM_STD_CAL_E,LAM_TAIL_CAL_E,LAM_DENSE_Q_CAL_E,0.0,0.0,0.0
        reweight_enabled=USE_THETA_REWEIGHT and (ep+1)>=THETA_REWEIGHT_START_EPOCH and stage in ["C","D","E"]
        combined_theta_weights=theta_problem_weights*theta_sampling_weights; combined_theta_weights/=combined_theta_weights.mean()+1e-9
        for g in opt.param_groups: g['lr']=cur_lr
        perm=rng.permutation(n); beta=BETA_KL_MAX*min(1.0,(ep+1)/KL_WARMUP_EPOCHS); acc=np.zeros(16)
        for b in range(nb):
            ...
            henc,w,mu,s=net.forward_gmm(xmu,th,sample=True); yh_n=(yh-hm)/(hs+1e-9); nll=(-gmm_log_prob(yh_n,w,mu,s)).mean(); p0h,q0h,pgh,qgh,thh=net.recover_boundary_dispatch_from_h_theta(henc,xr,th,yh,hm,hs,tm,ts,sample=True)
            p0q0_sup=((p0h-yp0)/p0s).pow(2).mean()+((q0h-yq0)/q0s).pow(2).mean(); t_sup=((thh-yt)/(ts+1e-9)).pow(2).mean(); bsup=p0q0_sup+LAM_T_SUP*t_sup; dsup=((pgh-ypg)/(net.pg_max_t-net.pg_min_t+1e-6)).pow(2).mean()+((qgh-yqg)/(net.qg_max_t-net.qg_min_t+1e-6)).pow(2).mean(); phys=physics_loss_flex(case,xr_raw,p0h,q0h,pgh,qgh,p_scale=p0s,q_scale=q0s); h_hat=th[:,0:1]*p0h+th[:,1:2]*q0h; scons=(((h_hat-yh)/(hs+1e-9))**2).mean()
            hq=torch.tensor(0.0,device=DEVICE); hcdf=torch.tensor(0.0,device=DEVICE)
            if USE_H_QUANTILE_LOSS and (ep+1)>=H_QUANTILE_START_EPOCH and cur_lam_hq>0 and (b % cur_q_every == 0): hq=compute_h_quantile_loss(net,XMU_tr,THETA_FEAT,QH_EMP_TR,xmu_mean,xmu_std,h_mean,h_std,taus=H_QUANTILE_TAUS,n_scenarios_sample=cur_q_scen,n_thetas_sample=cur_q_theta,sample=True,rng=np.random.default_rng(SEED_TRAIN + 100000*ep + b),theta_sampling_weights=(theta_sampling_weights if reweight_enabled else None))
            if cur_lam_hcdf>0: hcdf=compute_h_cdf_loss(net,XMU_tr,THETA_FEAT,YH_tr,xmu_mean,xmu_std,h_mean,h_std,n_scenarios_sample=cur_q_scen,n_thetas_sample=cur_q_theta,sample=True,rng=np.random.default_rng(SEED_TRAIN+200000*ep+b),theta_sampling_weights=(theta_sampling_weights if reweight_enabled else None))
            ...
            if USE_LOC_SCALE_CALIBRATION and (cur_lam_loc>0 or cur_lam_mean>0 or cur_lam_scale>0 or cur_lam_std>0 or cur_lam_tail>0):
                _,cal_parts=compute_h_loc_scale_calibration_loss(net,XMU_tr,THETA_FEAT,QH_CAL_EMP_TR,H_MEAN_EMP_TR,H_STD_EMP_TR,xmu_mean,xmu_std,h_mean,h_std,n_scenarios_sample=cur_q_scen,n_thetas_sample=cur_q_theta,sample=(False if CALIBRATION_USE_DETERMINISTIC_MEAN else True),rng=np.random.default_rng(SEED_TRAIN + 300000*ep + b),theta_sampling_weights=(theta_sampling_weights if reweight_enabled else None),theta_problem_weights=combined_theta_weights,taus=CALIB_TAUS,return_parts=False)
                h_loc_cal,h_mean_cal,h_scale_cal,h_std_cal,h_tail_cal=cal_parts["loc"],cal_parts["mean"],cal_parts["scale"],cal_parts["std"],cal_parts["tail"]
            theta_affine_reg=(net.theta_mu_bias.pow(2).mean()+net.theta_log_scale.pow(2).mean()) if USE_THETA_AFFINE_CALIBRATION else torch.tensor(0.0,device=DEVICE)
            kl=net.kl_divergence(); loss=nll+cur_lam_boundary*bsup+cur_lam_dispatch*dsup+cur_lam_phys*phys+LAM_SUPPORT_CONSIST*scons+cur_lam_hq*hq+cur_lam_hcdf*hcdf+cur_lam_loc*h_loc_cal+cur_lam_mean*h_mean_cal+cur_lam_scale*h_scale_cal+cur_lam_std*h_std_cal+cur_lam_tail*h_tail_cal+LAM_THETA_AFFINE_REG*theta_affine_reg+beta*kl/n
            opt.zero_grad(); loss.backward(); torch.nn.utils.clip_grad_norm_(net.parameters(),5.0); opt.step()
            acc += np.array([float(loss.detach().cpu()),float(nll.detach().cpu()),float(bsup.detach().cpu()),float(p0q0_sup.detach().cpu()),float(t_sup.detach().cpu()),float(dsup.detach().cpu()),float(phys.detach().cpu()),float(scons.detach().cpu()),float(hq.detach().cpu()),float(hcdf.detach().cpu()),float(h_loc_cal.detach().cpu()),float(h_mean_cal.detach().cpu()),float(h_scale_cal.detach().cpu()),float(h_std_cal.detach().cpu()),float(h_tail_cal.detach().cpu()),float(theta_affine_reg.detach().cpu())])
        if ep==0 or (ep+1)%50==0 or ep+1==EPOCHS:
            with torch.no_grad():
                henc,w,mu,s=net.forward_gmm(xmu_tv,th_tv,sample=False); nllv=(-gmm_log_prob((yh_tv-hm)/(hs+1e-9),w,mu,s)).mean(); p0h,q0h,pgh,qgh,thh=net.recover_boundary_dispatch_from_h_theta(henc,xr_tv,th_tv,yh_tv,hm,hs,tm,ts,sample=False)
                p0q0sv=((p0h-yp0_tv)/p0s).pow(2).mean()+((q0h-yq0_tv)/q0s).pow(2).mean(); tsv=((thh-yt_tv)/(ts+1e-9)).pow(2).mean(); bsv=p0q0sv+LAM_T_SUP*tsv; phv,parts=physics_loss_flex(case,xr_raw_v,p0h,q0h,pgh,qgh,return_parts=True,p_scale=p0s,q_scale=q0s)
                hqv=compute_h_quantile_loss(net,XMU_va,THETA_FEAT,QH_EMP_VAL,xmu_mean,xmu_std,h_mean,h_std,taus=H_QUANTILE_TAUS,n_scenarios_sample=min(cur_q_scen,max(1,XMU_va.shape[0])),n_thetas_sample=cur_q_theta,sample=False,rng=np.random.default_rng(SEED_TRAIN+999)) if (USE_H_QUANTILE_LOSS and cur_lam_hq>0) else torch.tensor(0.0,device=DEVICE)
                hcdfv=compute_h_cdf_loss(net,XMU_va,THETA_FEAT,YH_va,xmu_mean,xmu_std,h_mean,h_std,n_scenarios_sample=min(cur_q_scen,max(1,XMU_va.shape[0])),n_thetas_sample=cur_q_theta,sample=False,rng=np.random.default_rng(SEED_TRAIN+9999)) if cur_lam_hcdf>0 else torch.tensor(0.0,device=DEVICE)
                _,calv_parts=compute_h_loc_scale_calibration_loss(net,XMU_va,THETA_FEAT,QH_CAL_EMP_VAL,H_MEAN_EMP_VAL,H_STD_EMP_VAL,xmu_mean,xmu_std,h_mean,h_std,n_scenarios_sample=min(cur_q_scen,max(1,XMU_va.shape[0])),n_thetas_sample=cur_q_theta,sample=False,rng=np.random.default_rng(SEED_TRAIN+8888),theta_sampling_weights=None,theta_problem_weights=combined_theta_weights,taus=CALIB_TAUS,return_parts=False)
                val_loc_cal,val_mean_cal,val_scale_cal,val_std_cal,val_tail_cal=calv_parts["loc"],calv_parts["mean"],calv_parts["scale"],calv_parts["std"],calv_parts["tail"]
            print(f"Epoch {ep+1:4d} | stage={stage} | lr={cur_lr:.2e} | cur_lam_loc={cur_lam_loc:.3f} cur_lam_mean={cur_lam_mean:.3f} cur_lam_scale={cur_lam_scale:.3f} cur_lam_std={cur_lam_std:.3f} cur_lam_tail={cur_lam_tail:.3f} cur_lam_boundary={cur_lam_boundary:.3f} cur_lam_phys={cur_lam_phys:.3f} | avg_loc_cal={acc[10]/nb:.6f} avg_mean_cal={acc[11]/nb:.6f} avg_scale_cal={acc[12]/nb:.6f} avg_std_cal={acc[13]/nb:.6f} avg_tail_cal={acc[14]/nb:.6f} | val_loc_cal={val_loc_cal.item():.6f} val_mean_cal={val_mean_cal.item():.6f} val_scale_cal={val_scale_cal.item():.6f} val_std_cal={val_std_cal.item():.6f} val_tail_cal={val_tail_cal.item():.6f}")
            th_cdf=np.array([float(compute_h_cdf_loss(net,XMU_va,THETA_FEAT,YH_va,xmu_mean,xmu_std,h_mean,h_std,n_scenarios_sample=min(16,max(1,XMU_va.shape[0])),n_thetas_sample=1,sample=False,rng=np.random.default_rng(SEED_TRAIN+7000+j),theta_sampling_weights=np.eye(T)[j])) for j in range(T)])
            if reweight_enabled and ((ep+1)%THETA_REWEIGHT_UPDATE_EVERY==0):
                w=(th_cdf+1e-9)**THETA_REWEIGHT_POWER; w=w/(w.mean()+1e-9); w=np.clip(w,THETA_REWEIGHT_MIN,THETA_REWEIGHT_MAX); theta_sampling_weights=w; combined_theta_weights=theta_problem_weights*theta_sampling_weights; combined_theta_weights/=combined_theta_weights.mean()+1e-9
                print(f"[calib-theta-weights] updated combined weights = {np.round(combined_theta_weights,3).tolist()}")
            if USE_VAL_ARMS_DYNAMIC_REWEIGHT and (ep+1)>=VAL_ARMS_REWEIGHT_START_EPOCH and ((ep+1)%VAL_ARMS_REWEIGHT_UPDATE_EVERY==0):
                ...
                raw_w=(arms/(arms.mean()+1e-9))**VAL_ARMS_REWEIGHT_POWER; raw_w=np.clip(raw_w,VAL_ARMS_REWEIGHT_MIN,VAL_ARMS_REWEIGHT_MAX); raw_w=raw_w/(raw_w.mean()+1e-9)
                theta_sampling_weights=VAL_ARMS_REWEIGHT_SMOOTH*theta_sampling_weights + (1-VAL_ARMS_REWEIGHT_SMOOTH)*raw_w
                theta_sampling_weights=theta_sampling_weights/(theta_sampling_weights.mean()+1e-9)
                combined_theta_weights=theta_problem_weights*theta_sampling_weights; combined_theta_weights=combined_theta_weights/(combined_theta_weights.mean()+1e-9)
                worst=np.argsort(-arms)[:3].tolist(); print(f"[val-arms-reweight] epoch={ep+1} arms={np.round(arms,3).tolist()} raw_weights={np.round(raw_w,3).tolist()} theta_sampling_weights={np.round(theta_sampling_weights,3).tolist()} combined_theta_weights={np.round(combined_theta_weights,3).tolist()} worst3={worst}")
                val_arms_mean_recent=float(arms.mean()); val_arms_max_recent=float(arms.max())
            vscore=0.5*val_arms_mean_recent/100.0+0.3*val_arms_max_recent/100.0+float(hcdfv.item())+float(hqv.item())+float(val_loc_cal.item())+float(val_scale_cal.item())+0.5*float(val_tail_cal.item())+0.2*float(bsv.item())+0.2*float(phv.item()); print(f"[best-score] vscore={vscore:.6f} val_arms_mean={val_arms_mean_recent:.4f} val_arms_max={val_arms_max_recent:.4f} best_score={best_score:.6f}")
            if (ep+1)%50==0:
                b=net.theta_mu_bias.detach().cpu().numpy(); sc=np.exp(np.clip(net.theta_log_scale.detach().cpu().numpy(),-THETA_AFFINE_SCALE_CLAMP,THETA_AFFINE_SCALE_CLAMP)); print(f"[theta-affine] bias = {np.round(b,4).tolist()}"); print(f"[theta-affine] scale = {np.round(sc,4).tolist()}")
            if vscore < best_score - 1e-6: best_score=vscore; no_improve=0; best_state={k:v.detach().cpu().clone() for k,v in net.state_dict().items()}
            else: no_improve += 1
    ...
    train_info={"XMU_val":XMU_va,"YH_val":YH_va,"THETA_FEAT":THETA_FEAT,"theta_list":THETA_LIST,"cached_val_arms_csv":"cached_val_cdf_arms_v8.csv"}
    return net,norm,train_info
...
def eval_cached_validation_cdf_arms(net,norm,XMU_val,THETA_FEAT,theta_list,YH_val,save_prefix="cached_val_cdf_arms_v8",n_grid=300,n_posterior_samples=0):
    ...
    for j,th in enumerate(theta_list):
        ...
        for i in range(XMU_val.shape[0]):
            ...
            q50_mc=float(np.quantile(ys,0.5)); q05_mc=float(np.quantile(ys,0.05)); q95_mc=float(np.quantile(ys,0.95)); q50_l.append(q50_bp-q50_mc); sp_l.append((q95_bp-q05_bp)-(q95_mc-q05_mc))
            mu_mix=np.sum(wp*mp); var_mix=np.sum(wp*(sp*sp+mp*mp))-mu_mix*mu_mix; std_bp=np.sqrt(max(var_mix,1e-12))*hs; std_mc=np.std(ys); sr_l.append(std_bp/(std_mc+1e-9))
        rows.append({"theta_idx":j,"theta":float(th),"arms_pct":float(np.mean(arms_l)),"ks_stat":float(np.mean(ks_l)),"q50_err":float(np.mean(q50_l)),"span95_err":float(np.mean(sp_l)),"std_ratio":float(np.mean(sr_l)),"q05_err":float(np.mean([0.0])),"q95_err":float(np.mean([0.0])),"mean_err":float(np.mean(q50_l)),"std_err":float(np.mean([abs(x-1.0) for x in sr_l])),"primary_hint":"location" if abs(float(np.mean(q50_l)))>abs(float(np.mean(sp_l))) else "scale"})
...
def main():
    ...
    save_training_artifacts(net,norm,{
        ...
        "dataset_cache_mode":DATASET_CACHE_MODE,"dataset_version":DATASET_VERSION,"USE_LOC_SCALE_CALIBRATION":USE_LOC_SCALE_CALIBRATION,"CALIB_TAUS":CALIB_TAUS,"LAM_LOC_CAL_A":LAM_LOC_CAL_A,"LAM_LOC_CAL_B":LAM_LOC_CAL_B,"LAM_LOC_CAL_C":LAM_LOC_CAL_C,"LAM_LOC_CAL_D":LAM_LOC_CAL_D,"LAM_SCALE_CAL_A":LAM_SCALE_CAL_A,"LAM_SCALE_CAL_B":LAM_SCALE_CAL_B,"LAM_SCALE_CAL_C":LAM_SCALE_CAL_C,"LAM_SCALE_CAL_D":LAM_SCALE_CAL_D,"LAM_TAIL_CAL_A":LAM_TAIL_CAL_A,"LAM_TAIL_CAL_B":LAM_TAIL_CAL_B,"LAM_TAIL_CAL_C":LAM_TAIL_CAL_C,"LAM_TAIL_CAL_D":LAM_TAIL_CAL_D,"LAM_MEAN_CAL_A":LAM_MEAN_CAL_A,"LAM_MEAN_CAL_B":LAM_MEAN_CAL_B,"LAM_MEAN_CAL_C":LAM_MEAN_CAL_C,"LAM_MEAN_CAL_D":LAM_MEAN_CAL_D,"LAM_STD_CAL_A":LAM_STD_CAL_A,"LAM_STD_CAL_B":LAM_STD_CAL_B,"LAM_STD_CAL_C":LAM_STD_CAL_C,"LAM_STD_CAL_D":LAM_STD_CAL_D,"USE_STATIC_ERROR_TYPE_WEIGHTS":USE_STATIC_ERROR_TYPE_WEIGHTS,"LOCATION_THETA_INIT":LOCATION_THETA_INIT,"SCALE_THETA_INIT":SCALE_THETA_INIT,"SHAPE_THETA_INIT":SHAPE_THETA_INIT,"LOC_THETA_WEIGHT":LOC_THETA_WEIGHT,"SCALE_THETA_WEIGHT":SCALE_THETA_WEIGHT,"SHAPE_THETA_WEIGHT":SHAPE_THETA_WEIGHT,"DEFAULT_THETA_WEIGHT":DEFAULT_THETA_WEIGHT,"USE_STAGE_D_FINE_CALIBRATION":USE_STAGE_D_FINE_CALIBRATION,"FAST_STAGE_A_EPOCHS":FAST_STAGE_A_EPOCHS,"FAST_STAGE_B_EPOCHS":FAST_STAGE_B_EPOCHS,"FAST_STAGE_C_EPOCHS":FAST_STAGE_C_EPOCHS,"FAST_STAGE_D_EPOCHS":FAST_STAGE_D_EPOCHS,"USE_THETA_AFFINE_CALIBRATION":USE_THETA_AFFINE_CALIBRATION,"THETA_AFFINE_SCALE_CLAMP":THETA_AFFINE_SCALE_CLAMP,"LAM_THETA_AFFINE_REG":LAM_THETA_AFFINE_REG,"CALIBRATION_USE_DETERMINISTIC_MEAN":CALIBRATION_USE_DETERMINISTIC_MEAN,"USE_VAL_ARMS_DYNAMIC_REWEIGHT":USE_VAL_ARMS_DYNAMIC_REWEIGHT,"VAL_ARMS_REWEIGHT_START_EPOCH":VAL_ARMS_REWEIGHT_START_EPOCH,"VAL_ARMS_REWEIGHT_UPDATE_EVERY":VAL_ARMS_REWEIGHT_UPDATE_EVERY,"VAL_ARMS_REWEIGHT_POWER":VAL_ARMS_REWEIGHT_POWER,"VAL_ARMS_REWEIGHT_MIN":VAL_ARMS_REWEIGHT_MIN,"VAL_ARMS_REWEIGHT_MAX":VAL_ARMS_REWEIGHT_MAX,"VAL_ARMS_REWEIGHT_SMOOTH":VAL_ARMS_REWEIGHT_SMOOTH,"VAL_ARMS_FAST_GRID":VAL_ARMS_FAST_GRID
    })
