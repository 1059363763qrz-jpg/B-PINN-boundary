"""v7.4.6.6 rerun OPF q50 reference and compare selected-seed domains.
...
Windows PowerShell:
    py -m py_compile Po_flex_domain_v7_4_6_6_rerun_opf_q50_reference_compare.py
    py Po_flex_domain_v7_4_6_6_rerun_opf_q50_reference_compare.py
"""
...
def load_or_generate_mc_realizations(case, seed: int, cache: Dict[str, object]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, str, str]:
    global _any_sampling_mismatch_warning
    aliases = [
    ...
    if RERUN_OPF_Q50_USE_EXISTING_MC_REALIZATIONS_IF_AVAILABLE:
        for keys in aliases:
            if all(k in cache for k in keys):
                pd_mc, qd_mc, pr_mc, qr_mc = [np.asarray(cache[k], dtype=float)[:RERUN_OPF_Q50_N_MC] for k in keys]
    ...
    pd_rows, qd_rows, pr_rows, qr_rows = [], [], [], []
    for mc_idx in range(RERUN_OPF_Q50_N_MC):
        pdv, qdv, prv, qrv = v7463._draw_realization(case, pd_mu, qd_mu, pr_mu, qr_mu)
        pd_rows.append(pdv)
        qd_rows.append(qdv)
        pr_rows.append(prv)
        qr_rows.append(qrv)
...
def main() -> None:
    print("[v7.4.6.6-rerun-opf-q50-reference-compare] start", flush=True)
    print(f"[rerun-opf-q50-config] RUN_RERUN_OPF_Q50_REFERENCE={RUN_RERUN_OPF_Q50_REFERENCE}", flush=True)
    print(f"[rerun-opf-q50-config] seed={RERUN_OPF_Q50_SEED} n_mc={RERUN_OPF_Q50_N_MC} force={RERUN_OPF_Q50_FORCE_RECOMPUTE}", flush=True)
