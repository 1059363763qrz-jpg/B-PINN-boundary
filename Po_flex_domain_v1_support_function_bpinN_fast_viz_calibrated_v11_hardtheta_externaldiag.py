...
LOCATION_THETA_INIT = [7, 8, 6, 5, 3, 2, 10, 11]
...
EXTRA_HARD_THETA_INIT = [7, 8, 6, 5, 3, 2]
...
TRAINING_RUN_TAG = "v11_hardtheta_externaldiag"
...
def eval_all_theta_cdf_arms_multiscenario(case,net,norm,theta_list,n_eval_scenarios=10,mc_eval_per_scenario=400,seed=SEED_EVAL,cache_path="all_theta_eval_cache_multiscen_v9.npz",rebuild_cache=True,save_prefix="all_theta_multiscen_v11"):
...
def generate_worst_theta_diagnostics(pair_csv,theta_csv,save_path='worst_theta_diagnostics_v11.csv'):
    import pandas as pd
...
def compare_v11_against_v10(v11_pair_csv='cdf_error_decomp_multiscen_v11_by_pair.csv'):
    import pandas as pd
    base='all_theta_multiscen_v9_by_scenario_theta.csv' if Path('all_theta_multiscen_v9_by_scenario_theta.csv').exists() else 'cdf_error_decomp_multiscen_v10_by_pair.csv'
    if (not Path(v11_pair_csv).exists()) or (not Path(base).exists()):
        print('[v11-compare] skipped (missing csv).'); return
    b=pd.read_csv(base); n=pd.read_csv(v11_pair_csv)
    cb='arms_pct' if 'arms_pct' in b.columns else 'arms_v10'
    m=b[['scenario_idx','theta_idx',cb]].rename(columns={cb:'arms_v10'}).merge(n[['scenario_idx','theta_idx','arms_pct']].rename(columns={'arms_pct':'arms_v11'}),on=['scenario_idx','theta_idx'],how='inner')
    m['delta_arms']=m['arms_v11']-m['arms_v10']; m['improved']=(m['delta_arms']<0).astype(int)
    m.to_csv('v11_vs_v10_multiscen_comparison.csv',index=False)
    print('[v11-compare]')
    ...
    print(f"num improved pairs = {int((m['delta_arms']<0).sum())}")
    print(f"num worsened pairs = {int((m['delta_arms']>0).sum())}")
    wt0=int(m.groupby('theta_idx')['arms_v10'].mean().idxmax()); wt1=int(m.groupby('theta_idx')['arms_v11'].mean().idxmax())
    print(f"worst theta before/after = {wt0}/{wt1}")
def main():
    ...
    if RUN_TRAINING:
        ...
        if RUN_EVAL_ONLY:
            cv=eval_cached_validation_cdf_arms(net,norm,XMU,THETA_FEAT,THETA_LIST,YH,save_prefix="cached_val_cdf_arms_v11")
            ...
            with open('training_results/v11_hardtheta_externaldiag_metrics.json','w',encoding='utf-8') as f: json.dump(metrics,f,ensure_ascii=False,indent=2)
    ...
    if RUN_FULL_OPF_EVAL:
        ...
        if MULTI_SCENARIO_FORMAL_EVAL:
            eval_pack=eval_all_theta_cdf_arms_multiscenario(case,net,norm,THETA_LIST,n_eval_scenarios=N_FORMAL_EVAL_SCENARIOS,mc_eval_per_scenario=MC_EVAL_PER_SCENARIO,cache_path=FORMAL_EVAL_CACHE_PATH,rebuild_cache=FORMAL_EVAL_REBUILD_CACHE,save_prefix="all_theta_multiscen_v11")
    ...
    if MULTI_SCENARIO_FORMAL_EVAL and Path(FORMAL_EVAL_CACHE_PATH).exists():
        ...
        pair_df,_,_,_=analyze_cdf_error_decomposition_multiscenario(net,norm,XMU_eval,THETA_FEAT,THETA_LIST,YH_eval,save_prefix='cdf_error_decomp_multiscen_v11',cdf_grid_size=300,n_posterior_samples=1)
        scenario_src='all_theta_multiscen_v9_by_scenario_summary.csv' if Path('all_theta_multiscen_v9_by_scenario_summary.csv').exists() else 'cdf_error_decomp_multiscen_v11_by_scenario.csv'
        diagnose_scenario_coverage_shift(XMU,XMU_eval,scenario_src,norm,save_path='scenario_generalization_diagnostics_v10.csv')
        theta_src='all_theta_multiscen_v9_by_theta_summary.csv' if Path('all_theta_multiscen_v9_by_theta_summary.csv').exists() else 'cdf_error_decomp_multiscen_v11_by_theta.csv'
        generate_worst_theta_diagnostics('cdf_error_decomp_multiscen_v11_by_pair.csv',theta_src,save_path='worst_theta_diagnostics_v11.csv')
        check_multiscen_decomp_consistency('cdf_error_decomp_multiscen_v11_by_pair.csv','all_theta_multiscen_v9_by_scenario_theta.csv')
        plot_worst_pair_cdfs('cdf_error_decomp_multiscen_v11_by_pair.csv',net,norm,XMU_eval,THETA_FEAT,THETA_LIST,YH_eval)
