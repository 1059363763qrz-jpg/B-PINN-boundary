...
def eval_all_theta_cdf_arms(case,net,norm,theta_list,mc_eval=400,save_cache_path='all_theta_eval_cache.npz'):
    ...
    for j,th in enumerate(theta_list):
        ...
        for _ in range(mc_eval):
            ...
            if np.isfinite(v): ys.append(v)
        if len(ys)<30: continue
        if len(YH_eval)==0: YH_eval=[[] for _ in range(len(theta_list))]
        YH_eval[j]=np.array(ys,dtype=float)
    ...
    try:
        # store per-theta raw MC samples for decomposition
        np.savez(save_cache_path, XMU_eval=xmu, theta_list=np.array(theta_list), **{f'YH_theta_{k}':np.asarray(YH_eval[k]) for k in range(len(theta_list)) if len(np.asarray(YH_eval[k]).shape)>0})
    except Exception as e:
        print(f'[all-theta-arms] warning: failed to save eval cache: {e}')
    ...
    print(f"[all-theta-arms] mean={arr.mean():.4f}%")
    print(f"[all-theta-arms] median={np.median(arr):.4f}%")
    print(f"[all-theta-arms] max={arr.max():.4f}%")
    print(f"[all-theta-arms] q90={np.quantile(arr,0.9):.4f}%")
    print(f"[all-theta-arms] worst3={[w[0] for w in worst3]}")
    with open('all_theta_cdf_arms_v1.csv','w',newline='',encoding='utf-8') as f:
...
def analyze_cdf_error_decomposition(net,norm,XMU_eval,THETA_FEAT,theta_list,YH_eval,atom_diag_csv_path=None,active_diag_csv_path=None,taus=(0.01,0.05,0.10,0.25,0.50,0.75,0.90,0.95,0.99),cdf_grid_size=300,n_posterior_samples=30,save_prefix="cdf_error_decomp"):
    ...
    for j,th in enumerate(theta_list):
        ...
        xmu0=XMU_eval.mean(axis=0,keepdims=True)
        ...
        with torch.no_grad(): _,w,mu,s=net.forward_gmm(xt,th_t,sample=False); qn=gmm_quantile_torch(w,mu,s,list(taus)).cpu().numpy().reshape(-1)
        qbp=hmean+hstd*qn
        bp_mean_norm=(w*mu).sum(dim=1).cpu().numpy()[0]; bp_var_norm=(w*(s*s+mu*mu)).sum(dim=1).cpu().numpy()[0]-bp_mean_norm**2
        bp_mean=hmean+hstd*bp_mean_norm; bp_std=abs(hstd)*np.sqrt(max(0.0,bp_var_norm)); mc_mean=float(np.mean(h_mc)); mc_std=float(np.std(h_mc))
        errs=qbp-qmc
...
def main():
    ...
    if Path('all_theta_eval_cache.npz').exists():
        ...
        XMU_eval=np.repeat(d['XMU_eval'],YH.shape[0],axis=0)
        YH_eval=np.zeros((YH.shape[0],MC_EVAL_MULTI,THETA_FEAT.shape[0]))
        for j in range(THETA_FEAT.shape[0]):
            key=f'YH_theta_{j}'
            if key in d: YH_eval[:, :len(d[key])//max(1,YH.shape[0]), j]=np.nanmean(d[key])
            else: YH_eval[:,:,j]=np.nan
        YH_eval=np.where(np.isfinite(YH_eval),YH_eval,np.nanmean(YH[:,:,j]))
    else:
