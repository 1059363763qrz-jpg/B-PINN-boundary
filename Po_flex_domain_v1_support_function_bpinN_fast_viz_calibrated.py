...
if RUN_MODE == "smoke":
    ...
    NUM_SCENARIOS, MC_PER_SCENARIO, N_THETA, EPOCHS = 300, 30, 12, 180
    RUN_MULTI_TEST=True; MC_EVAL_MULTI=400; EVAL_THETA_SAMPLES=40
    USE_H_QUANTILE_LOSS=True; LAM_H_QUANTILE=0.05
elif RUN_MODE == "full":
...
FAST_STAGE_A_EPOCHS = 100
FAST_STAGE_B_EPOCHS = 80
FAST_TOTAL_EPOCHS = FAST_STAGE_A_EPOCHS + FAST_STAGE_B_EPOCHS
...
def compute_h_quantile_loss(net,XMU_scen,THETA_FEAT,QH_EMP,x_mu_mean,x_mu_std,h_mean,h_std,taus=H_QUANTILE_TAUS,n_scenarios_sample=H_QUANTILE_BATCH_SCENARIOS,n_thetas_sample=H_QUANTILE_BATCH_THETAS,sample=True,rng=None):
    ...
    theta_idx=np.arange(n_theta) if n_thetas_sample is None else rng.choice(n_theta,size=min(n_thetas_sample,n_theta),replace=False)
...
def compute_h_cdf_loss(net,XMU_scen,THETA_FEAT,YH_scen,x_mu_mean,x_mu_std,h_mean,h_std,n_scenarios_sample=8,n_thetas_sample=4,sample=True,rng=None):
    ...
    z=torch.tensor(Z_CDF_GRID,dtype=torch.float32,device=DEVICE).view(1,-1,1)
    hs=YH_scen[s_flat,:,t_flat]; hs_n=(hs-h_mean)/(h_std+1e-9)
    emp=np.mean(hs_n[:,:,None] <= Z_CDF_GRID[None,None,:],axis=1)
...
def train_bayes_flex_gmm2(case,XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG):
    ...
    for ep in range(EPOCHS):
        stage = "A" if (ep+1)<=FAST_STAGE_A_EPOCHS else "B"
        cur_q_every = 5 if stage=="A" else 1
        cur_q_scen = 8 if stage=="A" else 32
        cur_q_theta = 4 if stage=="A" else None
        cur_lam_hq = 0.05 if stage=="A" else 0.15
        cur_lam_hcdf = 0.0 if stage=="A" else 0.15
        cur_lr = LR if stage=="A" else LR*0.3
        ...
        for b in range(nb):
            ...
            if USE_H_QUANTILE_LOSS and (ep+1)>=H_QUANTILE_START_EPOCH and (b % cur_q_every == 0):
                hq_rng=np.random.default_rng(SEED_TRAIN + 100000*ep + b)
                hq = compute_h_quantile_loss(net,XMU_tr,THETA_FEAT,QH_EMP_TR,xmu_mean,xmu_std,h_mean,h_std,taus=H_QUANTILE_TAUS,n_scenarios_sample=cur_q_scen,n_thetas_sample=cur_q_theta,sample=True,rng=hq_rng)
            hcdf = compute_h_cdf_loss(net,XMU_tr,THETA_FEAT,YH_tr,xmu_mean,xmu_std,h_mean,h_std,n_scenarios_sample=cur_q_scen,n_thetas_sample=cur_q_theta,sample=True,rng=np.random.default_rng(SEED_TRAIN+200000*ep+b))
        ...
        if ep==0 or (ep+1)%50==0 or ep+1==EPOCHS:
            ...
            print(f"Epoch {ep+1:4d} | stage={stage} | lr={cur_lr:.2e} | gmmK={N_GMM_COMPONENTS} | lam_hq={cur_lam_hq:.3f} lam_hcdf={cur_lam_hcdf:.3f} | avg_loss={acc[0]/nb:.6f} avg_nll_h={acc[1]/nb:.6f} avg_boundary_sup={acc[2]/nb:.6f} avg_t_sup={acc[4]/nb:.6f} avg_phys_flex={acc[6]/nb:.6f} avg_h_quantile={acc[8]/nb:.6f} avg_h_cdf={acc[9]/nb:.6f} | val_h_quantile={hqv.item():.6f} val_h_cdf={hcdfv.item():.6f} val_boundary_sup={bsv.item():.6f} val_phys_flex={phv.item():.6f} val_raw_pcc_p={parts['raw_pcc_p'].item():.3e} val_raw_pcc_q={parts['raw_pcc_q'].item():.3e}")
...
def eval_and_plot_flex_domain(case,net,norm,theta_list,mc_eval=500,seed=SEED_EVAL):
    ...
    if len(poly_mc95)>2: x,y=close(poly_mc95); plt.fill(x,y,color='#9aa4b2',alpha=0.18,label='MC 95% quantile domain')
    if len(poly_mc05)>2: x,y=close(poly_mc05); plt.fill(x,y,color='#6b7280',alpha=0.20,label='MC 5% quantile domain')
    if len(poly_bn95)>2: x,y=close(poly_bn95); plt.fill(x,y,color='#f6c28b',alpha=0.18,label='B-PINN 95% quantile domain')
    if len(poly_bn05)>2: x,y=close(poly_bn05); plt.fill(x,y,color='#f59e0b',alpha=0.15,label='B-PINN 5% quantile domain')
    if len(poly_mc50)>2: x,y=close(poly_mc50); plt.plot(x,y,'-',color='#374151',lw=2.6,label='MC median domain')
    if len(poly_bn50)>2: x,y=close(poly_bn50); plt.plot(x,y,'--',color='#d97706',lw=2.6,label='B-PINN median domain')
    ...
    plt.grid(alpha=0.25,color='0.7'); plt.xlabel('P0 (MW)',fontsize=12); plt.ylabel('Q0 (Mvar)',fontsize=12); plt.title('IEEE 33-bus Flexibility Domain — support-function B-PINN'); plt.legend(loc='upper right',fontsize=9); plt.axis('equal'); plt.margins(0.08); plt.tight_layout(); plt.savefig('FlexDomain_33bus_v1_support_function_full.png',dpi=280)
...
def eval_all_theta_cdf_arms(case,net,norm,theta_list,mc_eval=400):
    ...
    if not rows: return
    arr=np.array([r[2] for r in rows]); worst=max(rows,key=lambda x:x[2]); print(f"[all-theta-arms] mean={arr.mean():.4f}% median={np.median(arr):.4f}% max={arr.max():.4f}% worst_idx={worst[0]} theta={worst[1]:.4f}")
    with open('all_theta_cdf_arms_v1.csv','w',newline='',encoding='utf-8') as f:
        w=csv.writer(f); w.writerow(['theta_idx','theta','arms_pct']); [w.writerow(r) for r in rows]
    plt.figure(figsize=(8,3.8),dpi=140); plt.bar([r[0] for r in rows],[r[2] for r in rows],color='#6b7280'); plt.xlabel('theta idx'); plt.ylabel('ARMS (%)'); plt.title('All-theta CDF ARMS'); plt.tight_layout(); plt.savefig('FlexDomain_all_theta_ARMS_bar_v1.png',dpi=250)
...
def eval_and_plot_flex_domain_posterior(case,net,norm,theta_list,n_post=30,mc_eval=200):
    ...
    plt.title('Posterior variability of B-PINN median flexibility domain'); plt.xlabel('P0 (MW)'); plt.ylabel('Q0 (Mvar)'); plt.grid(alpha=0.25); plt.legend(); plt.axis('equal'); plt.tight_layout(); plt.savefig('FlexDomain_33bus_posterior_overlay_v1.png',dpi=260)
...
def eval_and_plot_realization_cloud(case,net,norm,theta_list,n_real=24):
    ...
    plt.title('Realization cloud of flexibility domains'); plt.xlabel('P0 (MW)'); plt.ylabel('Q0 (Mvar)'); plt.grid(alpha=0.25); plt.axis('equal'); plt.tight_layout(); plt.savefig('FlexDomain_33bus_realization_cloud_v1.png',dpi=260)
...
def main():
    ...
    eval_all_theta_cdf_arms(case,net,norm,THETA_LIST)
