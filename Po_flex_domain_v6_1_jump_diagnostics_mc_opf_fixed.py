...
TRAINING_RESULT_DIR = "training_results_theta_independent_v7style"
RUN_TRAINING = True
RUN_EVAL_ONLY = False
...
ALL_THETA_TRAIN = True
ALL_THETA_EVAL = True
ALL_THETA_FLEX_SYNTHESIS = True
ALL_THETA_RESULT_DIR = "training_results_theta_independent_v7style_alltheta"
ALL_THETA_COMPARISON_DIR = "training_results_theta_independent_v7style_alltheta_comparison"
ALL_THETA_FLEX_DIR = "training_results_theta_independent_v7style_flex_synthesis"
TRAIN_V7_MODEL_IF_MISSING = True
SKIP_TRAIN_IF_MODEL_EXISTS = True
ALL_THETA_EVAL_SEEDS = [1,2,3]
ALL_THETA_EVAL_MC = 800
...
# ===== v6.1 jump diagnostics =====
DIAG_OUTPUT_DIR = "diagnostics_jump"
RUN_TRUE_TRUNC_COMPARISON = True
DIAG_CASES = [
    {"theta_idx": 3, "seed": 1, "mc": 2500},
    {"theta_idx": 3, "seed": 3, "mc": 2500},
    {"theta_idx": 2, "seed": 1, "mc": 2500},
    {"theta_idx": 5, "seed": 1, "mc": 2500},
]
DIAG_PROGRESS_EVERY = 100
ATOM_TOLS = [1e-5,1e-4,5e-4,1e-3,5e-3]
WINDOW_WIDTHS = [1e-4,5e-4,1e-3,5e-3,1e-2]

def sample_clip_normal_with_flag(mu, sigma, lo=0.0, hi=None):
    raw=np.random.normal(mu,sigma); clipped_low=raw<lo; clipped_high=(hi is not None) and (raw>hi); x=max(lo,raw)
    if hi is not None: x=min(hi,x)
    return float(x),float(raw),bool(clipped_low),bool(clipped_high)

def sample_true_trunc_normal_rejection(mu, sigma, lo=0.0, hi=None, max_try=10000):
    raw=mu
    for _ in range(max_try):
        raw=np.random.normal(mu,sigma)
        if raw>=lo and (hi is None or raw<=hi): return float(raw)
    x=max(lo,raw)
    if hi is not None: x=min(hi,x)
    return float(x)

def draw_diag_external_scenario(case, seed):
    rng=np.random.default_rng(seed); return sample_scenario_means(case,rng)

def run_raw_sampling_diagnostic(case,pd_mu,qd_mu,pr_mu,qr_mu,seed,mc,sampler="clip",theta_idx=-1):
    np.random.seed(seed+777); rows=[]
    for m in range(mc):
        pdv=pd_mu.copy(); prv=pr_mu.copy(); std_pd=0.10*np.maximum(pd_mu,1e-3); std_pr=0.12*np.maximum(pr_mu,1e-3)
        nload=0
        for i in range(case.n_bus):
            if pd_mu[i]>1e-9:
                if sampler=="clip":
                    val,raw,cl,ch=sample_clip_normal_with_flag(pd_mu[i],std_pd[i],0.0,None); pdv[i]=val; nload += int(cl)
                else: pdv[i]=sample_true_trunc_normal_rejection(pd_mu[i],std_pd[i],0.0,None)
        pvvals=[]; pvraw=[]; pvl=[]; pvh=[]
        for k,b in enumerate(case.pv_buses):
            if sampler=="clip":
                val,raw,cl,ch=sample_clip_normal_with_flag(pr_mu[b],std_pr[b],0.0,float(case.pv_pmax[k])); prv[b]=val
            else:
                val=sample_true_trunc_normal_rejection(pr_mu[b],std_pr[b],0.0,float(case.pv_pmax[k])); raw=val; cl=False; ch=False; prv[b]=val
            pvvals.append(val); pvraw.append(raw); pvl.append(int(cl)); pvh.append(int(ch))
        rows.append({'theta_idx':theta_idx,'seed':seed,'mc':mc,'sampler':sampler,'sample_idx':m,'pd_sum':float(np.sum(pdv)),'pr_sum':float(np.sum(prv)),'num_load_clip_low':nload,'num_pv_clip_low':int(np.sum(pvl)),'num_pv_clip_high':int(np.sum(pvh)),
                     'pv0_value':pvvals[0],'pv1_value':pvvals[1],'pv2_value':pvvals[2],'pv3_value':pvvals[3],'pv0_raw':pvraw[0],'pv1_raw':pvraw[1],'pv2_raw':pvraw[2],'pv3_raw':pvraw[3],
                     'pv0_clip_low':pvl[0],'pv1_clip_low':pvl[1],'pv2_clip_low':pvl[2],'pv3_clip_low':pvl[3],'pv0_clip_high':pvh[0],'pv1_clip_high':pvh[1],'pv2_clip_high':pvh[2],'pv3_clip_high':pvh[3]})
    Path(DIAG_OUTPUT_DIR).mkdir(parents=True,exist_ok=True); df=pd.DataFrame(rows); df.to_csv(f"{DIAG_OUTPUT_DIR}/raw_sampling_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}_samples.csv",index=False)
    sm={'theta_idx':theta_idx,'seed':seed,'mc':mc,'sampler':sampler}
    for i in range(4): sm[f'pv{i}_clip_low_rate']=float(df[f'pv{i}_clip_low'].mean()); sm[f'pv{i}_clip_high_rate']=float(df[f'pv{i}_clip_high'].mean())
    sm.update({'mean_num_pv_clip_high':float(df['num_pv_clip_high'].mean()),'mean_num_pv_clip_low':float(df['num_pv_clip_low'].mean()),'mean_num_load_clip_low':float(df['num_load_clip_low'].mean())})
    pd.DataFrame([sm]).to_csv(f"{DIAG_OUTPUT_DIR}/raw_sampling_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}_summary.csv",index=False)
    return df,sm

def run_mc_opf_jump_diagnostic(case,theta_idx,theta_list,seed,mc,sampler="clip"):
    pd_mu,qd_mu,pr_mu,qr_mu=draw_diag_external_scenario(case,seed); theta=float(theta_list[theta_idx]); alpha=float(np.cos(theta)); beta=float(np.sin(theta)); rows=[]; succ=0; infeas=0; t0=time.perf_counter(); np.random.seed(seed+100000+theta_idx); signature_to_id={}; next_signature_id=0
    for m in range(mc):
        pdv=pd_mu.copy(); prv=pr_mu.copy(); std_pd=0.10*np.maximum(pd_mu,1e-3); std_pr=0.12*np.maximum(pr_mu,1e-3); nload=0; pvraw=[]; pvvals=[]; pvl=[]; pvh=[]
        for i in range(case.n_bus):
            if pd_mu[i]>1e-9:
                if sampler=="clip": val,raw,cl,ch=sample_clip_normal_with_flag(pd_mu[i],std_pd[i],0.0,None); pdv[i]=val; nload += int(cl)
                else: pdv[i]=sample_true_trunc_normal_rejection(pd_mu[i],std_pd[i],0.0,None)
        for k,b in enumerate(case.pv_buses):
            if sampler=="clip": val,raw,cl,ch=sample_clip_normal_with_flag(pr_mu[b],std_pr[b],0.0,float(case.pv_pmax[k])); prv[b]=val
            else: val=sample_true_trunc_normal_rejection(pr_mu[b],std_pr[b],0.0,float(case.pv_pmax[k])); raw=val; cl=False; ch=False; prv[b]=val
            pvraw.append(raw); pvvals.append(val); pvl.append(int(cl)); pvh.append(int(ch))
        qdv=qd_mu*(pdv/np.maximum(pd_mu,1e-6)); qrv=prv*math.tan(math.acos(case.pv_pf))
        sol=solve_flex_support_gurobi_33bus(case,pdv,qdv,prv,qrv,alpha,beta,return_detail=True,stabilize_dispatch=STABILIZE_OPF_DISPATCH)
        rec={'theta_idx':theta_idx,'theta':theta,'seed':seed,'sample_idx':m,'sampler':sampler,'ok':int(sol.get('ok',False)),'pd_sum':float(np.sum(pdv)),'pr_sum':float(np.sum(prv)),'num_pv_clip_low':int(np.sum(pvl)),'num_pv_clip_high':int(np.sum(pvh)),'num_load_clip_low':int(nload),
             'pv0_raw':pvraw[0],'pv1_raw':pvraw[1],'pv2_raw':pvraw[2],'pv3_raw':pvraw[3],'pv0_value':pvvals[0],'pv1_value':pvvals[1],'pv2_value':pvvals[2],'pv3_value':pvvals[3],'pv0_clip_low':pvl[0],'pv1_clip_low':pvl[1],'pv2_clip_low':pvl[2],'pv3_clip_low':pvl[3],'pv0_clip_high':pvh[0],'pv1_clip_high':pvh[1],'pv2_clip_high':pvh[2],'pv3_clip_high':pvh[3]}
        if sol.get('ok',False):
            succ+=1; sig,act,names=get_active_constraint_signature(case,sol)
            if sig not in signature_to_id:
                signature_to_id[sig]=next_signature_id; next_signature_id+=1
            sig_id=signature_to_id[sig]
            rec.update({'h':float(sol.get('h',np.nan)),'h_stage1':float(sol.get('h_stage1',np.nan)),'P0':float(sol.get('P0',np.nan)),'Q0':float(sol.get('Q0',np.nan)),'support_residual':float(abs(sol.get('h',np.nan)-alpha*sol.get('P0',0)-beta*sol.get('Q0',0))),
                        'Pg0':float(sol.get('Pg',np.full(4,np.nan))[0]),'Pg1':float(sol.get('Pg',np.full(4,np.nan))[1]),'Pg2':float(sol.get('Pg',np.full(4,np.nan))[2]),'Pg3':float(sol.get('Pg',np.full(4,np.nan))[3]),
                        'Qg0':float(sol.get('Qg',np.full(4,np.nan))[0]),'Qg1':float(sol.get('Qg',np.full(4,np.nan))[1]),'Qg2':float(sol.get('Qg',np.full(4,np.nan))[2]),'Qg3':float(sol.get('Qg',np.full(4,np.nan))[3]),
                        'Vmin':float(np.min(sol.get('V',np.array([np.nan])))),'Vmax':float(np.max(sol.get('V',np.array([np.nan])))),'Pline_abs_max':float(np.max(np.abs(sol.get('P',np.array([np.nan]))))),
                        'Qline_abs_max':float(np.max(np.abs(sol.get('Q',np.array([np.nan]))))),'active_signature_id':int(sig_id),'active_names':';'.join(act),'active_signature_bits':' '.join(str(int(x)) for x in sig)})
        else:
            infeas+=1
            rec.update({'h':np.nan,'h_stage1':np.nan,'P0':np.nan,'Q0':np.nan,'support_residual':np.nan,'Pg0':np.nan,'Pg1':np.nan,'Pg2':np.nan,'Pg3':np.nan,'Qg0':np.nan,'Qg1':np.nan,'Qg2':np.nan,'Qg3':np.nan,'Vmin':np.nan,'Vmax':np.nan,'Pline_abs_max':np.nan,'Qline_abs_max':np.nan,'active_signature_id':-1,'active_names':'','active_signature_bits':''})
        rows.append(rec)
        if (m+1)%DIAG_PROGRESS_EVERY==0 or (m+1)==mc:
            el=time.perf_counter()-t0; eta=(mc-m-1)*(el/max(1,m+1)); print(f"[jump-mc-opf] theta_idx={theta_idx:02d} seed={seed:03d} sampler={sampler} {m+1}/{mc} success={succ} infeasible={infeas} elapsed={el:.1f}s eta={eta:.1f}s",flush=True)
    df=pd.DataFrame(rows); Path(DIAG_OUTPUT_DIR).mkdir(parents=True,exist_ok=True); df.to_csv(f"{DIAG_OUTPUT_DIR}/mc_opf_diag_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}_samples.csv",index=False); return df

def analyze_h_jump_distribution(samples_df,theta_idx,seed,mc,sampler):
    d=samples_df[samples_df['ok']==1].copy(); h=np.sort(d['h'].values.astype(float)); n=len(h)
    sm={'theta_idx':theta_idx,'seed':seed,'mc':mc,'sampler':sampler,'n_success':n,'h_min':float(np.min(h)),'h_q01':float(np.quantile(h,0.01)),'h_q05':float(np.quantile(h,0.05)),'h_q25':float(np.quantile(h,0.25)),'h_q50':float(np.quantile(h,0.5)),'h_q75':float(np.quantile(h,0.75)),'h_q95':float(np.quantile(h,0.95)),'h_q99':float(np.quantile(h,0.99)),'h_max':float(np.max(h)),'h_mean':float(np.mean(h)),'h_std':float(np.std(h))}
    atom_rows=[]
    for tol in ATOM_TOLS:
        hb=np.round(h/tol)*tol; vc=pd.Series(hb).value_counts().sort_values(ascending=False); maxc=int(vc.iloc[0]); sm[f'max_bin_mass_tol{tol}']=float(maxc/max(1,n))
        top=vc.head(5).reset_index(); top.columns=['h_bin','count']; top['mass']=top['count']/max(1,n); top['tol']=tol; atom_rows.append(top)
        top.to_csv(f"{DIAG_OUTPUT_DIR}/h_atom_bins_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}_tol{tol:.0e}.csv",index=False)
    atom_df=pd.concat(atom_rows,ignore_index=True)
    pd.DataFrame([sm]).to_csv(f"{DIAG_OUTPUT_DIR}/h_atom_summary_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}.csv",index=False)
    win=[]
    for w in WINDOW_WIDTHS:
        j=0; best=0; bl=h[0]; br=h[0]
        for i in range(n):
            while j<n and h[j]-h[i]<=w: j+=1
            c=j-i
            if c>best: best=c; bl=h[i]; br=h[j-1]
        win.append({'window':w,'max_window_count':best,'max_window_mass':best/max(1,n),'window_left':bl,'window_right':br})
    wdf=pd.DataFrame(win); wdf.to_csv(f"{DIAG_OUTPUT_DIR}/h_window_mass_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}.csv",index=False)
    return sm,atom_df,wdf

def analyze_active_constraints(samples_df,atom_bins_df,theta_idx,seed,mc,sampler):
    d=samples_df[samples_df['ok']==1].copy(); g=d.groupby('active_signature_id').agg(count=('h','count'),mean_h=('h','mean'),std_h=('h','std'),h_min=('h','min'),h_max=('h','max'),mean_num_pv_clip_high=('num_pv_clip_high','mean'),mean_num_pv_clip_low=('num_pv_clip_low','mean'),top_active_names=('active_names',lambda x: x.mode().iat[0] if len(x.mode()) else '')).reset_index(); g['mass']=g['count']/max(1,len(d)); g=g.sort_values('count',ascending=False)
    g.to_csv(f"{DIAG_OUTPUT_DIR}/active_signature_summary_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}.csv",index=False)
    top_bin=float(atom_bins_df.iloc[0]['h_bin']); tol=float(atom_bins_df.iloc[0]['tol']); near=d[np.abs(d['h']-top_bin)<=max(tol,1e-4)]
    ng=near.groupby('active_signature_id').agg(count=('h','count'),mass=('h',lambda x: len(x)/max(1,len(near))),mean_num_pv_clip_high=('num_pv_clip_high','mean'),mean_num_pv_clip_low=('num_pv_clip_low','mean'),top_active_names=('active_names',lambda x: x.mode().iat[0] if len(x.mode()) else '')).reset_index()
    ng.to_csv(f"{DIAG_OUTPUT_DIR}/active_signature_near_top_atom_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}.csv",index=False)
    return g

def plot_jump_diagnostics(samples_df,theta_idx,seed,mc,sampler,wdf):
    d=samples_df[samples_df['ok']==1].copy(); h=np.sort(d['h'].values.astype(float)); cdf=np.arange(1,len(h)+1)/max(1,len(h)); row=wdf.iloc[wdf['max_window_mass'].idxmax()]
    plt.figure(figsize=(7,5)); plt.plot(h,cdf,'k-'); plt.axvspan(row['window_left'],row['window_right'],color='red',alpha=0.2); plt.tight_layout(); plt.savefig(f"{DIAG_OUTPUT_DIR}/cdf_jump_annotated_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}.png",dpi=220); plt.close()
    plt.figure(figsize=(7,4)); plt.hist(d['h'].values,bins=60); plt.tight_layout(); plt.savefig(f"{DIAG_OUTPUT_DIR}/h_hist_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}.png",dpi=220); plt.close()
    plt.figure(figsize=(7,4)); plt.scatter(d['h'].values,d['num_pv_clip_high'].values,s=8,alpha=0.4); plt.tight_layout(); plt.savefig(f"{DIAG_OUTPUT_DIR}/h_vs_pv_clip_high_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}.png",dpi=220); plt.close()
    dd=d.copy(); dd['sig']=dd['active_signature_id'].fillna(-1); plt.figure(figsize=(7,4)); plt.scatter(np.arange(len(dd)),dd['h'].values,c=dd['sig'],s=8,cmap='tab20'); plt.tight_layout(); plt.savefig(f"{DIAG_OUTPUT_DIR}/h_by_active_signature_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}.png",dpi=220); plt.close()
    vc=dd['active_signature_id'].value_counts().head(15); plt.figure(figsize=(8,4)); plt.bar(vc.index.astype(str),vc.values); plt.xticks(rotation=90); plt.tight_layout(); plt.savefig(f"{DIAG_OUTPUT_DIR}/active_signature_bar_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_{sampler}.png",dpi=220); plt.close()

def run_sampler_comparison_for_case(case,theta_idx,seed,mc,clip_df=None):
    if clip_df is None:
        clip_df=run_mc_opf_jump_diagnostic(case,theta_idx,THETA_LIST,seed,mc,sampler='clip')
    csm,cat,cw=analyze_h_jump_distribution(clip_df,theta_idx,seed,mc,'clip')
    tru_df=run_mc_opf_jump_diagnostic(case,theta_idx,THETA_LIST,seed,mc,sampler='true_trunc'); tsm,tat,tw=analyze_h_jump_distribution(tru_df,theta_idx,seed,mc,'true_trunc')
    agc=analyze_active_constraints(clip_df,cat,theta_idx,seed,mc,'clip'); agt=analyze_active_constraints(tru_df,tat,theta_idx,seed,mc,'true_trunc')
    summ=pd.DataFrame([{'sampler':'clip','h_mean':csm['h_mean'],'h_std':csm['h_std'],'max_bin_mass_tol1e-4':csm.get('max_bin_mass_tol0.0001',np.nan),'max_window_mass_1e-3':float(cw.loc[cw['window']==1e-3,'max_window_mass'].values[0]),'max_window_mass_5e-3':float(cw.loc[cw['window']==5e-3,'max_window_mass'].values[0]),'top_active_signature_mass':float(agc['mass'].max() if len(agc) else np.nan),'mean_num_pv_clip_high':float(clip_df[clip_df.ok==1]['num_pv_clip_high'].mean()),'mean_num_pv_clip_low':float(clip_df[clip_df.ok==1]['num_pv_clip_low'].mean())},
                       {'sampler':'true_trunc','h_mean':tsm['h_mean'],'h_std':tsm['h_std'],'max_bin_mass_tol1e-4':tsm.get('max_bin_mass_tol0.0001',np.nan),'max_window_mass_1e-3':float(tw.loc[tw['window']==1e-3,'max_window_mass'].values[0]),'max_window_mass_5e-3':float(tw.loc[tw['window']==5e-3,'max_window_mass'].values[0]),'top_active_signature_mass':float(agt['mass'].max() if len(agt) else np.nan),'mean_num_pv_clip_high':float(tru_df[tru_df.ok==1]['num_pv_clip_high'].mean()),'mean_num_pv_clip_low':float(tru_df[tru_df.ok==1]['num_pv_clip_low'].mean())}])
    summ.to_csv(f"{DIAG_OUTPUT_DIR}/sampler_compare_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_summary.csv",index=False)
    hc=np.sort(clip_df[clip_df.ok==1]['h'].values); ht=np.sort(tru_df[tru_df.ok==1]['h'].values); cc=np.arange(1,len(hc)+1)/len(hc); ct=np.arange(1,len(ht)+1)/len(ht)
    title='jump likely caused by clipping atom' if float(summ.iloc[0]['max_window_mass_1e-3'])>float(summ.iloc[1]['max_window_mass_1e-3'])*1.2 else 'jump likely caused by active constraints / degenerate critical region'
    plt.figure(figsize=(7,5)); plt.plot(hc,cc,label='clip empirical CDF'); plt.plot(ht,ct,label='true_trunc empirical CDF'); plt.title(title); plt.legend(); plt.tight_layout(); plt.savefig(f"{DIAG_OUTPUT_DIR}/sampler_compare_theta{theta_idx:02d}_seed{seed:03d}_mc{mc}_cdf.png",dpi=220); plt.close()

def main_jump_diagnostics():
    print('[jump-diagnostics] start',flush=True); case=build_ieee33_case(); Path(DIAG_OUTPUT_DIR).mkdir(parents=True,exist_ok=True)
    for item in DIAG_CASES:
        theta_idx=item['theta_idx']; seed=item['seed']; mc=item['mc']; theta=float(THETA_LIST[theta_idx]); alpha=float(np.cos(theta)); beta=float(np.sin(theta))
        print('[jump-diagnostics-case]',flush=True); print(f'theta_idx = {theta_idx}',flush=True); print(f'seed = {seed}',flush=True); print(f'mc = {mc}',flush=True); print(f'theta = {theta}',flush=True); print(f'alpha = {alpha}',flush=True); print(f'beta = {beta}',flush=True)
        pd_mu,qd_mu,pr_mu,qr_mu=draw_diag_external_scenario(case,seed); run_raw_sampling_diagnostic(case,pd_mu,qd_mu,pr_mu,qr_mu,seed,mc,sampler='clip',theta_idx=theta_idx)
        sc=run_mc_opf_jump_diagnostic(case,theta_idx,THETA_LIST,seed,mc,sampler='clip'); sm,ab,wdf=analyze_h_jump_distribution(sc,theta_idx,seed,mc,'clip'); ag=analyze_active_constraints(sc,ab,theta_idx,seed,mc,'clip'); plot_jump_diagnostics(sc,theta_idx,seed,mc,'clip',wdf)
        top_active_mass=float(ag['mass'].max() if len(ag) else 0.0); mb14=float(sm.get('max_bin_mass_tol0.0001',0.0)); mw13=float(wdf.loc[wdf['window']==1e-3,'max_window_mass'].values[0]); mpvh=float(sc[sc.ok==1]['num_pv_clip_high'].mean())
        if mb14>0.10 and mpvh>0.2: hint='clipping_atom_likely'
        elif mw13>0.15 and top_active_mass>0.50: hint='active_constraint_saturation_likely'
        elif mw13>0.15: hint='degenerate_or_low_sensitivity_region_likely'
        else: hint='no_strong_atom_detected'
        print('[jump-diagnostics-summary]',flush=True); print(f'theta_idx = {theta_idx}',flush=True); print(f'seed = {seed}',flush=True); print('sampler = clip',flush=True); print(f'n_success = {int(sm["n_success"])}',flush=True); print(f'n_infeasible = {int((sc.ok==0).sum())}',flush=True); print(f'h_mean = {sm["h_mean"]}',flush=True); print(f'h_std = {sm["h_std"]}',flush=True); print(f'max_bin_mass_tol1e-4 = {mb14}',flush=True); print(f'max_window_mass_1e-3 = {mw13}',flush=True); print(f'top_active_signature_mass = {top_active_mass}',flush=True); print(f'mean_num_pv_clip_high = {mpvh}',flush=True); print(f'mean_num_pv_clip_low = {float(sc[sc.ok==1]["num_pv_clip_low"].mean())}',flush=True); print(f'diagnosis_hint = {hint}',flush=True)
        if RUN_TRUE_TRUNC_COMPARISON: run_sampler_comparison_for_case(case,theta_idx,seed,mc,clip_df=sc)
    print('[jump-diagnostics] done',flush=True)

if __name__ == "__main__":
    main_jump_diagnostics()
