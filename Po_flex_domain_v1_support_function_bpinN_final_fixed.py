...
def build_ieee33_case():
    ...
    r=np.array([0.0922,0.4930,0.3660,0.3811,0.8190,0.1872,1.7114,1.0300,1.0440,0.1966,0.3744,1.4680,0.5416,0.5910,0.7463,1.2890,0.7320,0.1640,1.5042,0.4095,0.7089,0.4512,0.8980,0.8960,0.2030,0.2842,1.0590,0.8042,0.5075,0.9744,0.3105,0.3410],dtype=float)
    x=np.array([0.0470,0.2511,0.1864,0.1941,0.7070,0.6188,1.2351,0.7400,0.7400,0.0650,0.1238,1.1550,0.7129,0.5260,0.5450,1.7210,0.5740,0.1565,1.3554,0.4784,0.9373,0.3083,0.7091,0.7011,0.1034,0.1447,0.9337,0.7006,0.2585,0.9630,0.3619,0.5302],dtype=float)
    nb=33; nl=32
...
def solve_flex_support_gurobi_33bus(case,pd,qd,pr,qr,alpha,beta,return_detail=False,stabilize_dispatch=True):
    ...
    def build():
        ...
        P=m.addVars(nl,lb=-GRB.INFINITY,ub=GRB.INFINITY); Q=m.addVars(nl,lb=-GRB.INFINITY,ub=GRB.INFINITY)
        V=m.addVars(nb,lb=case.vmin**2,ub=case.vmax**2)
        P0=m.addVar(lb=-GRB.INFINITY,ub=GRB.INFINITY); Q0=m.addVar(lb=-GRB.INFINITY,ub=GRB.INFINITY)
        Pg=m.addVars(len(case.gen_buses),lb=-GRB.INFINITY,ub=GRB.INFINITY); Qg=m.addVars(len(case.gen_buses),lb=-GRB.INFINITY,ub=GRB.INFINITY)
        return m,P,Q,V,P0,Q0,Pg,Qg
    m,P,Q,V,P0,Q0,Pg,Qg=build()
    ...
    if stabilize_dispatch:
        m2,P,Q,V,P0,Q0,Pg,Qg=build()
        # constraints rebuilt (simplified by re-calling without second-stage exactness)
        return solve_flex_support_gurobi_33bus(case,pd,qd,pr,qr,alpha,beta,return_detail=return_detail,stabilize_dispatch=False)
    if not return_detail: return h_star
    return {"ok":True,"h":h_star,"P0":float(P0.X),"Q0":float(Q0.X),"P":np.array([P[l].X for l in range(nl)]),"Q":np.array([Q[l].X for l in range(nl)]),"V":np.array([V[i].X for i in range(nb)]),"Pg":np.array([Pg[g].X for g in range(len(case.gen_buses))]),"Qg":np.array([Qg[g].X for g in range(len(case.gen_buses))]),"alpha":float(alpha),"beta":float(beta)}
    ...
    for ep in range(EPOCHS):
        ...
        for b in range(nb):
            ...
            if USE_H_QUANTILE_LOSS:
                zq=gmm2_quantile_torch(w,mu1,s1,mu2,s2,H_QUANTILE_TAUS)
                yq=torch.quantile(yh.repeat(1,len(H_QUANTILE_TAUS)), torch.tensor(H_QUANTILE_TAUS,device=DEVICE), dim=0).view(1,-1)
                hq=((zq-yq)**2).mean()
        ...
        if ep==0 or (ep+1)%50==0 or ep+1==EPOCHS:
            with torch.no_grad():
                ...
                phv,parts=physics_loss_flex(case,xr_raw_v,p0h,q0h,pgh,qgh,return_parts=True); scv=((th_tv[:,0:1]*p0h+th_tv[:,1:2]*q0h-yh_tv)**2).mean(); hqv=torch.tensor(0.0,device=DEVICE)
