...
N_FORMAL_EVAL_SCENARIOS = 2   # quick check; for formal report use 10
MC_EVAL_PER_SCENARIO = 100    # quick check; for formal report use 400
...
def save_training_artifacts(net,norm,config_dict):
    ...
    tag=config_dict.get("training_run_tag_effective",TRAINING_RUN_TAG)
    mp=f"{TRAINING_RESULT_DIR}/{tag}_model.pt"
    npth=f"{TRAINING_RESULT_DIR}/{tag}_norm.pkl"
    cp=f"{TRAINING_RESULT_DIR}/{tag}_config.json"
...
def main():
    ...
    if RUN_TRAINING:
        ...
        print("[checkpoint] training mode uses timestamp checkpoint only; best overwrite disabled unless explicitly enabled.")
        save_training_artifacts(net,norm,{
            ...
            "training_run_tag_effective": f"{TRAINING_RUN_TAG}_{ts}"
        })
    else:
        ...
        if RUN_EVAL_ONLY:
            n_scen=XMU.shape[0]; n_val=max(1,int(0.1*n_scen)); idx=np.random.default_rng(SEED_TRAIN).permutation(n_scen); va=idx[-n_val:]
            print(f"[eval-only-cached-val] using validation split, n_val={n_val}")
            cv=eval_cached_validation_cdf_arms(net,norm,XMU[va],THETA_FEAT,THETA_LIST,YH[va],save_prefix="cached_val_cdf_arms_evalonly_v9")
