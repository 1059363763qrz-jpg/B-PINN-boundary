...
def build_ieee33_case():
    ...
    nl=fb.size; nb=33
    r=np.full(nl,0.01); x=np.full(nl,0.02)
    pd=np.full(nb,0.03); qd=np.full(nb,0.015); pd[0]=qd[0]=0
    pv_b=np.array([5,10,20,25],dtype=int); pv_max=np.array([0.35,0.35,0.40,0.40],dtype=float)
    gen_b=np.array([0,12,24],dtype=int)
    pg_min=np.array([0.0,0.0,0.0]); pg_max=np.array([3.0,1.5,1.5]); qg_min=np.array([-2.0,-1.0,-1.0]); qg_max=np.array([2.0,1.0,1.0])
    c,pb,pbu,to,rt,inb,outb = _build_radial_topology(nb,0,fb,tb)
    return GridCase(nb,0,fb,tb,r,x,0.95,1.05,np.full(nl,4.0),np.full(nl,4.0),pd,qd,pv_b,pv_max,0.98,gen_b,pg_min,pg_max,qg_min,qg_max,c,pb,pbu,to,rt,inb,outb)
