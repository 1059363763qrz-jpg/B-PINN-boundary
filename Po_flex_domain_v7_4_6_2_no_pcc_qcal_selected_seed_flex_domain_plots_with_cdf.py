...
Windows PowerShell:
    py Po_flex_domain_v7_4_6_2_no_pcc_qcal_selected_seed_flex_domain_plots_with_cdf.py
...
caches, flex-domain plots, per-theta CDF plots, and a visualization manifest.
"""
...
CDF_GRID_N = 500
CDF_PLOT_THETA_LIST = list(range(12))
...
def plot_theta_cdf_for_seed(seed, theta_idx, h_mc, gmm_params, theta_metrics_df, out_dir):
    ...
    if gmm_params.get("status") == "ok":
        ...
        plt.plot(x_grid, F_gmm, "r--", linewidth=2.0, label="BPINN GMM CDF")
    ...
    _save_fig(base_path, seed, f"Theta {theta_idx:02d} MC empirical CDF vs BPINN GMM CDF", png_artifact_type="theta_cdf_png", pdf_artifact_type="theta_cdf_pdf")
    plt.close()
...
def plot_theta_cdf_montage_for_seed(seed, supports, model_map, theta_metrics_df):
    ...
    for ax, theta_idx in zip(axes.ravel(), range(N_THETA)):
        ...
        if gmm.get("status") == "ok":
            ax.plot(x_grid, gmm_cdf_np(x_grid, gmm["weights"], gmm["mu_raw"], gmm["sigma_raw"]), "r--", linewidth=1.2, label="BPINN")
        arms = _theta_arms(seed, theta_idx, theta_metrics_df)
    ...
    fig.suptitle(f"Seed {seed:04d}: theta CDF montage", fontsize=14)
    fig.tight_layout(rect=[0, 0, 0.98, 0.97])
    _save_fig(seed_dir / f"selected_seed_{_seed_tag(seed)}_theta_cdf_montage", seed, "Montage of theta MC empirical CDFs vs BPINN GMM CDFs", png_artifact_type="theta_cdf_montage_png", pdf_artifact_type="theta_cdf_montage_pdf")
    plt.close(fig)
...
def save_theta_cdf_summary(seed, supports, model_map, theta_metrics_df):
    ...
    for theta_idx in range(N_THETA):
        ...
        rows.append({
            ...
            "mc_h_q05": float(np.nanquantile(h_mc, 0.05)) if len(h_mc) else np.nan,
            "mc_h_q50": float(np.nanquantile(h_mc, 0.50)) if len(h_mc) else np.nan,
            "mc_h_q95": float(np.nanquantile(h_mc, 0.95)) if len(h_mc) else np.nan,
    ...
    _safe_to_csv(df, out, index=False, description="Per-theta CDF summary")
    _record_artifact("csv", seed, out, "Per-theta CDF summary")
    return df
...
def plot_theta_cdfs_for_seed(seed, supports, model_map, theta_metrics_df):
    ...
    if SAVE_INDIVIDUAL_THETA_CDF:
        for theta_idx in CDF_PLOT_THETA_LIST:
            ...
            gmm = get_bpinn_gmm_raw_for_theta(seed, theta_idx, x_mu, model_map)
            plot_theta_cdf_for_seed(seed, theta_idx, h_mc, gmm, theta_metrics_df, seed_dir)
...
def main():
    ...
    manifest_csv = _abs_path(SELECTED_FLEX_DOMAIN_OUT_DIR) / "visualization_manifest_v7_4_6_2_selected_seed_flex_domain.csv"
