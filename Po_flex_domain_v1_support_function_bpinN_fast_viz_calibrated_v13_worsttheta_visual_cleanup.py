...
def compare_v13_against_v12(v13_pair_csv='cdf_error_decomp_multiscen_v13_by_pair.csv'):
    ...
    v12_src='cdf_error_decomp_multiscen_v12_by_pair.csv'
    if (not Path(v13_pair_csv).exists()) or (not Path(v12_src).exists()):
        print('[v12-compare] skipped (missing csv).'); return
    v12=pd.read_csv(v12_src); v13=pd.read_csv(v13_pair_csv)
    m=v12[['scenario_idx','theta_idx','arms_pct']].rename(columns={'arms_pct':'arms_v12'}).merge(v13[['scenario_idx','theta_idx','arms_pct']].rename(columns={'arms_pct':'arms_v13'}),on=['scenario_idx','theta_idx'])
    ...
    print('[v12-compare]')
    print(f"mean ARMS v13 = {m['arms_v10'].mean():.4f}")
    print(f"mean ARMS v13 = {m['arms_v11'].mean():.4f}")
    print(f"mean ARMS v13 = {m['arms_v12'].mean():.4f}")
    print(f"max ARMS v13 = {m['arms_v10'].max():.4f}")
    print(f"max ARMS v13 = {m['arms_v11'].max():.4f}")
    print(f"max ARMS v13 = {m['arms_v12'].max():.4f}")
    print(f"q90 ARMS v13 = {q90(m['arms_v10']):.4f}")
    print(f"q90 ARMS v13 = {q90(m['arms_v11']):.4f}")
    print(f"q90 ARMS v13 = {q90(m['arms_v12']):.4f}")
    print(f"num improved vs v12 = {int((m['delta_v12_vs_v10']<0).sum())}")
    print(f"num improved vs v12 = {int((m['delta_v12_vs_v11']<0).sum())}")
    print(f"num worsened vs v12 = {int((m['delta_v12_vs_v11']>0).sum())}")
...
def write_visualization_manifest_v13():
    rows=[
        ...
        ['all_theta_multiscen_v13_worst6_cdf_grid.png','worst pair CDF','plot_worst_pair_cdfs','外部多场景最差6个CDF对比','after formal cache eval','v13'],
        ['all_theta_multiscen_v13_worst_pair_cdf.png','worst pair CDF','plot_worst_pair_cdfs','外部多场景最差pair CDF对比','after formal cache eval','v13'],
...
def main():
    ...
    if MULTI_SCENARIO_FORMAL_EVAL and Path(FORMAL_EVAL_CACHE_PATH).exists():
        ...
        scenario_src='all_theta_multiscen_v9_by_scenario_summary.csv' if Path('all_theta_multiscen_v9_by_scenario_summary.csv').exists() else 'cdf_error_decomp_multiscen_v13_by_scenario.csv'
        diagnose_scenario_coverage_shift(XMU,XMU_eval,scenario_src,norm,save_path='scenario_generalization_diagnostics_v13.csv')
        theta_src='all_theta_multiscen_v9_by_theta_summary.csv' if Path('all_theta_multiscen_v9_by_theta_summary.csv').exists() else 'cdf_error_decomp_multiscen_v13_by_theta.csv'
