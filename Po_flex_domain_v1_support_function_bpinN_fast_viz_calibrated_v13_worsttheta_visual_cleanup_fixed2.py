...
LOCATION_THETA_INIT = [2, 0, 1, 3, 9, 11]
SCALE_THETA_INIT = [2, 0, 1, 3]
SHAPE_THETA_INIT = [2, 3, 10, 11]
LOC_THETA_WEIGHT = 2.5
SCALE_THETA_WEIGHT = 1.8
SHAPE_THETA_WEIGHT = 1.5
DEFAULT_THETA_WEIGHT = 1.0
EXTRA_HARD_THETA_INIT = [2]
EXTRA_HARD_THETA_WEIGHT = 3.5
MODERATE_HARD_THETA_INIT = [0, 1, 3, 9, 11]
MODERATE_HARD_THETA_WEIGHT = 2.2
STABLE_HARD_THETA_INIT = [7, 8, 6, 5]
...
USE_WORST_THETA_FINE_CAL = True
WORST_THETA_FINE_CAL = [2,0,1,3]
...
TRAINING_RUN_TAG = "v13_worsttheta_visual_cleanup"
...
def compute_worst_theta_fine_calibration_loss(net,XMU_scen,THETA_FEAT,QH_CAL_EMP,x_mu_mean,x_mu_std,h_mean,h_std,worst_theta_list,n_scenarios_sample,sample,rng,theta_problem_weights=None):
    ...
    for j in idx:
        ...
        with torch.no_grad() if False else torch.enable_grad():
            ...
            q=gmm_quantile_torch(w,mu,sig,[0.05,0.50,0.95])
        q50=h_mean+h_std*q[:,1:2]
        span=(h_mean+h_std*q[:,2:3])-(h_mean+h_std*q[:,0:1])
        qemp=np.asarray(QH_CAL_EMP)[scen_idx,j,:]
        tq50=torch.tensor(qemp[:,2:3],dtype=torch.float32,device=DEVICE)
        tspan=torch.tensor((qemp[:,3:4]-qemp[:,1:2]),dtype=torch.float32,device=DEVICE)
        loc_terms.append(torch.mean((q50-tq50)**2)); scale_terms.append(torch.mean((span-tspan)**2))
...
def train_bayes_flex_gmm2(case,XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG):
    ...
    print("[theta-problem-weights-v13]")
    for jj in range(T):
        bt="default"
    ...
    for ep in range(EPOCHS):
        ...
        for b in range(nb):
            ...
            if USE_WORST_THETA_FINE_CAL and stage in ["C", "D"]:
                worst_loc_cal, worst_scale_cal = compute_worst_theta_fine_calibration_loss(
                    net,
                    XMU_tr,
                    THETA_FEAT,
                    QH_CAL_EMP_TR,
                    ...
                    h_mean,
                    h_std,
                    worst_theta_list=WORST_THETA_FINE_CAL,
                    n_scenarios_sample=cur_q_scen,
                    sample=False,
                    rng=np.random.default_rng(SEED_TRAIN + 500000 * ep + b),
                    theta_problem_weights=combined_theta_weights,
                )
            ...
            if stage=='E':
                ...
                if stage == "C":
                    loss = loss + LAM_WORST_LOC_C * worst_loc_cal + LAM_WORST_SCALE_C * worst_scale_cal
                elif stage == "D":
                    loss = loss + LAM_WORST_LOC_D * worst_loc_cal + LAM_WORST_SCALE_D * worst_scale_cal
...
def eval_cached_validation_cdf_arms(net,norm,XMU_val,THETA_FEAT,theta_list,YH_val,save_prefix="cached_val_cdf_arms_v8",n_grid=300,n_posterior_samples=0):
    ...
    vals=np.array([r['arms_pct'] for r in rows]); worst=np.argsort(-vals)[:3].tolist(); print(f"[cached-val-arms-v13] mean={vals.mean():.4f}"); print(f"[cached-val-arms-v13] median={np.median(vals):.4f}"); print(f"[cached-val-arms-v13] max={vals.max():.4f}"); print(f"[cached-val-arms-v13] worst3={worst}")
...
def eval_and_plot_direction_cdfs(case,net,norm,theta_list):
    ...
    plt.tight_layout(); plt.savefig('FlexDomain_CDF_selected_directions_v13.png',dpi=260)
...
def eval_and_plot_flex_domain(case,net,norm,theta_list,mc_eval=500,seed=SEED_EVAL):
    ...
    if len(poly_mc95)>2:
        ...
        plt.grid(alpha=0.25); plt.title('MC quantile flexibility domains under source-load uncertainty'); plt.xlabel('P0 (MW)',fontsize=12); plt.ylabel('Q0 (Mvar)',fontsize=12); plt.axis('equal'); plt.legend(); plt.tight_layout(); plt.savefig('FlexDomain_MC_quantile_domains_v13.png',dpi=280)
    if len(poly_bn95)>2:
        ...
        plt.grid(alpha=0.25); plt.title('B-PINN predicted quantile flexibility domains'); plt.xlabel('P0 (MW)',fontsize=12); plt.ylabel('Q0 (Mvar)',fontsize=12); plt.axis('equal'); plt.legend(); plt.tight_layout(); plt.savefig('FlexDomain_BPINN_quantile_domains_v13.png',dpi=280)
...
def eval_and_plot_flex_domain_posterior(case,net,norm,theta_list,n_post=30,mc_eval=200):
    ...
    plt.title('B-PINN posterior uncertainty of median flexibility domain'); plt.xlabel('P0 (MW)'); plt.ylabel('Q0 (Mvar)'); plt.grid(alpha=0.25); plt.legend(); plt.axis('equal'); plt.tight_layout(); plt.savefig('FlexDomain_posterior_overlay_v13.png',dpi=280)
...
def eval_and_plot_realization_cloud(case,net,norm,theta_list,n_real=24):
    ...
    plt.title('Realization cloud of flexibility domains under source-load uncertainty'); plt.xlabel('P0 (MW)'); plt.ylabel('Q0 (Mvar)'); plt.grid(alpha=0.25); plt.legend(['MC realization domains','MC median domain','B-PINN median domain']); plt.axis('equal'); plt.tight_layout(); plt.savefig('FlexDomain_realization_cloud_v13.png',dpi=280)
...
def diagnose_support_distribution_atoms(YH,THETA_FEAT,theta_list,active_records=None,arms_csv_path=None,eps_list=(1e-4,1e-3,1e-2,5e-2),top_bins=3,save_prefix='support_atom_diagnostic'):
    ...
    for ax,k in zip(axs,pick):
        ...
        ax.grid(alpha=0.25)
    plt.tight_layout(); plt.savefig('support_distribution_histograms_worst_theta_v13.png',dpi=280)
    ...
    if valid:
        ...
        plt.grid(alpha=0.25); plt.xlabel('mass_eps_1e-2'); plt.ylabel('ARMS (%)'); plt.tight_layout(); plt.savefig('support_atom_vs_arms_scatter_v13.png',dpi=280)
...
def analyze_cdf_error_decomposition(net,norm,XMU_eval,THETA_FEAT,theta_list,YH_eval,atom_diag_csv_path=None,active_diag_csv_path=None,taus=(0.01,0.05,0.10,0.25,0.50,0.75,0.90,0.95,0.99),cdf_grid_size=300,n_posterior_samples=30,save_prefix="cdf_error_decomp"):
    ...
    plt.tight_layout(); plt.savefig('cdf_residual_worst_theta_v13.png',dpi=280)
    ...
    plt.grid(alpha=0.25); plt.tight_layout(); plt.savefig('cdf_atom_vs_error_scatter_v13.png',dpi=280)
    ...
    mx=max(max(x),max(y))+1e-9; plt.plot([0,mx],[0,mx],'k--'); plt.grid(alpha=0.25); plt.tight_layout(); plt.savefig('cdf_bias_vs_uncertainty_scatter_v13.png',dpi=280)
...
def diagnose_scenario_coverage_shift(XMU_train,XMU_eval,scenario_summary_csv,norm,save_path='scenario_generalization_diagnostics_v13.csv'):
    import pandas as pd
...
def generate_worst_theta_diagnostics(pair_csv,theta_csv,save_path='worst_theta_diagnostics_v13.csv'):
    import pandas as pd
...
def plot_worst_pair_cdfs(pair_csv,net,norm,XMU_eval,THETA_FEAT,theta_list,YH_eval,save_prefix="all_theta_multiscen_v13"):
...
def compare_v13_against_v12(v13_pair_csv='cdf_error_decomp_multiscen_v13_by_pair.csv'):
    if (not Path(v13_pair_csv).exists()) or (not Path('cdf_error_decomp_multiscen_v12_by_pair.csv').exists()):
        print('[v13-compare] skipped (missing csv).'); return
    v12=pd.read_csv('cdf_error_decomp_multiscen_v12_by_pair.csv')
    v13=pd.read_csv(v13_pair_csv)
    ...
    m['delta_v13_vs_v12']=m['arms_v13']-m['arms_v12']
    m['improved_vs_v12']=(m['delta_v13_vs_v12']<0).astype(int)
    m.to_csv('v13_vs_v12_multiscen_comparison.csv',index=False)
    ...
    print(f"num improved vs v12 = {int((m['delta_v13_vs_v12']<0).sum())}")
    print(f"num worsened vs v12 = {int((m['delta_v13_vs_v12']>0).sum())}")
    t2=m[m['theta_idx']==2]
    if len(t2)>0:
        print(f"theta=2 mean before/after = {t2['arms_v12'].mean():.4f}/{t2['arms_v13'].mean():.4f}")
        print(f"theta=2 max before/after = {t2['arms_v12'].max():.4f}/{t2['arms_v13'].max():.4f}")
    w6v12=np.sort(m['arms_v12'].values)[-6:]; w6v13=np.sort(m['arms_v13'].values)[-6:]
    print(f"worst6 mean before/after = {np.mean(w6v12):.4f}/{np.mean(w6v13):.4f}")

def finalize_v13_checkpoint(net,norm,cached_stats,external_stats):
    Path(TRAINING_RESULT_DIR).mkdir(parents=True,exist_ok=True)
    ts=datetime.now().strftime("%Y%m%d_%H%M%S")
    base=f"{TRAINING_RESULT_DIR}/{TRAINING_RUN_TAG}_{ts}"
    ...
    selection_score=float(external_stats['external_mean_arms']+0.50*external_stats['external_q90_arms']+0.30*external_stats['external_max_arms']+0.20*cached_stats['cached_val_mean_arms'])
    ...
    with open(f"{TRAINING_RESULT_DIR}/v13_worsttheta_visual_cleanup_metrics.json",'w',encoding='utf-8') as f: json.dump(metrics,f,ensure_ascii=False,indent=2)
    best_metrics_path=Path(TRAINING_RESULT_DIR)/'v13_worsttheta_visual_cleanup_best_metrics.json'
    ...
    if update_best:
        shutil.copy2(model_path,f"{TRAINING_RESULT_DIR}/v13_worsttheta_visual_cleanup_best_model.pt")
        shutil.copy2(norm_path,f"{TRAINING_RESULT_DIR}/v13_worsttheta_visual_cleanup_best_norm.pkl")
        shutil.copy2(config_path,f"{TRAINING_RESULT_DIR}/v13_worsttheta_visual_cleanup_best_config.json")
        with open(best_metrics_path,'w',encoding='utf-8') as f: json.dump(metrics,f,ensure_ascii=False,indent=2)
    print('[v13-checkpoint]')
...
def write_visualization_manifest_v13():
    rows=[
['cached_val_cdf_arms_v13.csv','cached validation','eval_cached_validation_cdf_arms','每个theta验证ARMS表','after training','v13'],
['cached_val_cdf_arms_v13_bar.png','cached validation','eval_cached_validation_cdf_arms','每个theta验证ARMS柱状图','after training','v13'],
['FlexDomain_CDF_selected_directions_v13.png','flexdomain','eval_and_plot_flex_domain','方向CDF对比','if enabled','v13'],
['FlexDomain_overlay_quantile_comparison_v13.png','flexdomain','eval_and_plot_flex_domain','分位域叠加对比','if enabled','v13'],
['FlexDomain_MC_quantile_domains_v13.png','flexdomain','eval_and_plot_flex_domain','MC分位域','if enabled','v13'],
['FlexDomain_BPINN_quantile_domains_v13.png','flexdomain','eval_and_plot_flex_domain','BPINN分位域','if enabled','v13'],
['FlexDomain_all_theta_ARMS_bar_v13.png','flexdomain','eval_all_theta_cdf_arms_multiscenario','全theta ARMS柱状图','after formal eval','v13'],
['FlexDomain_posterior_overlay_v13.png','flexdomain','eval_and_plot_flex_domain_posterior','后验叠加','if enabled','v13'],
['FlexDomain_realization_cloud_v13.png','flexdomain','eval_and_plot_realization_cloud','实现云图','if enabled','v13'],
['support_atom_mass_by_theta_v13.png','atom diagnostic','diagnose_support_distribution_atoms','atom质量','always','v13'],
['support_scene_atom_ratio_by_theta_v13.png','atom diagnostic','diagnose_support_distribution_atoms','场景atom比例','always','v13'],
['support_distribution_histograms_worst_theta_v13.png','atom diagnostic','diagnose_support_distribution_atoms','最差theta分布直方图','always','v13'],
['support_atom_vs_arms_scatter_v13.png','atom diagnostic','diagnose_support_distribution_atoms','atom与ARMS散点','always','v13'],
['cdf_error_type_by_theta_v13.png','cdf decomp','analyze_cdf_error_decomposition','error type柱状图','after decomp','v13'],
['cdf_quantile_error_heatmap_v13.png','cdf decomp','analyze_cdf_error_decomposition','分位误差热力图','after decomp','v13'],
['cdf_location_scale_tail_bar_v13.png','cdf decomp','analyze_cdf_error_decomposition','loc/scale/tail条形图','after decomp','v13'],
['cdf_residual_worst_theta_v13.png','cdf decomp','analyze_cdf_error_decomposition','最差theta残差','after decomp','v13'],
['cdf_atom_vs_error_scatter_v13.png','cdf decomp','analyze_cdf_error_decomposition','atom vs error','after decomp','v13'],
['cdf_bias_vs_uncertainty_scatter_v13.png','cdf decomp','analyze_cdf_error_decomposition','bias vs uncertainty','after decomp','v13'],
['all_theta_multiscen_v13_worst_pair_cdf.png','worst pair CDF','plot_worst_pair_cdfs','最差pair CDF','after formal cache eval','v13'],
['all_theta_multiscen_v13_worst6_cdf_grid.png','worst pair CDF','plot_worst_pair_cdfs','最差6个CDF','after formal cache eval','v13'],
['cdf_error_decomp_multiscen_v13_by_pair.csv','cdf decomp','analyze_cdf_error_decomposition_multiscenario','pair级误差表','after formal cache eval','v13'],
['cdf_error_decomp_multiscen_v13_by_theta.csv','cdf decomp','analyze_cdf_error_decomposition_multiscenario','theta汇总','after formal cache eval','v13'],
['cdf_error_decomp_multiscen_v13_by_scenario.csv','cdf decomp','analyze_cdf_error_decomposition_multiscenario','scenario汇总','after formal cache eval','v13'],
['cdf_error_decomp_multiscen_v13_summary.csv','cdf decomp','analyze_cdf_error_decomposition_multiscenario','总体汇总','after formal cache eval','v13'],
['worst_theta_diagnostics_v13.csv','diagnostic','generate_worst_theta_diagnostics','worst theta诊断','after formal cache eval','v13'],
['scenario_generalization_diagnostics_v13.csv','diagnostic','diagnose_scenario_coverage_shift','场景覆盖诊断','after formal cache eval','v13'],
['v13_vs_v12_multiscen_comparison.csv','comparison','compare_v13_against_v12','v13和v12外部多场景对比','after formal cache eval','v13']
]
    import csv
    with open('visualization_manifest_v13.csv','w',newline='',encoding='utf-8') as f:
        w=csv.writer(f); w.writerow(['filename','category','function_name','purpose','generated_when','version_status']); w.writerows(rows)
    print('[visualization-manifest-v13] saved visualization_manifest_v13.csv')
...
def main():
    ...
    if RUN_TRAINING:
        ...
        if RUN_EVAL_ONLY:
            cv=eval_cached_validation_cdf_arms(net,norm,XMU,THETA_FEAT,THETA_LIST,YH,save_prefix="cached_val_cdf_arms_v13")
            ...
            with open('training_results/v13_worsttheta_visual_cleanup_metrics.json','w',encoding='utf-8') as f: json.dump(metrics,f,ensure_ascii=False,indent=2)
    ...
    if RUN_FULL_OPF_EVAL:
        ...
        if MULTI_SCENARIO_FORMAL_EVAL:
            eval_pack=eval_all_theta_cdf_arms_multiscenario(case,net,norm,THETA_LIST,n_eval_scenarios=N_FORMAL_EVAL_SCENARIOS,mc_eval_per_scenario=MC_EVAL_PER_SCENARIO,cache_path=FORMAL_EVAL_CACHE_PATH,rebuild_cache=FORMAL_EVAL_REBUILD_CACHE,save_prefix="all_theta_multiscen_v13")
    ...
    cached_cv=eval_cached_validation_cdf_arms(net,norm,XMU,THETA_FEAT,THETA_LIST,YH,save_prefix='cached_val_cdf_arms_v13')
    ...
    print(f"[cached-val-v13] mean={cached_stats['cached_val_mean_arms']:.4f} median={float(np.nanmedian(cv_vals)):.4f} max={cached_stats['cached_val_max_arms']:.4f} worst3={cached_stats['cached_val_worst3']}")
    ...
    if MULTI_SCENARIO_FORMAL_EVAL and Path(FORMAL_EVAL_CACHE_PATH).exists():
        ...
        pair_df,_,_,_=analyze_cdf_error_decomposition_multiscenario(net,norm,XMU_eval,THETA_FEAT,THETA_LIST,YH_eval,save_prefix='cdf_error_decomp_multiscen_v13',cdf_grid_size=300,n_posterior_samples=EVAL_THETA_SAMPLES)
        scenario_src='cdf_error_decomp_multiscen_v13_by_scenario.csv' if Path('cdf_error_decomp_multiscen_v13_by_scenario.csv').exists() else ('all_theta_multiscen_v9_by_scenario_summary.csv' if Path('all_theta_multiscen_v9_by_scenario_summary.csv').exists() else 'cdf_error_decomp_multiscen_v13_by_scenario.csv')
        diagnose_scenario_coverage_shift(XMU,XMU_eval,scenario_src,norm,save_path='scenario_generalization_diagnostics_v13.csv')
        theta_src='cdf_error_decomp_multiscen_v13_by_theta.csv' if Path('cdf_error_decomp_multiscen_v13_by_theta.csv').exists() else ('all_theta_multiscen_v9_by_theta_summary.csv' if Path('all_theta_multiscen_v9_by_theta_summary.csv').exists() else 'cdf_error_decomp_multiscen_v13_by_theta.csv')
        generate_worst_theta_diagnostics('cdf_error_decomp_multiscen_v13_by_pair.csv',theta_src,save_path='worst_theta_diagnostics_v13.csv')
        plot_worst_pair_cdfs('cdf_error_decomp_multiscen_v13_by_pair.csv',net,norm,XMU_eval,THETA_FEAT,THETA_LIST,YH_eval)
        compare_v13_against_v12('cdf_error_decomp_multiscen_v13_by_pair.csv')
        ...
        ext_df=pd.read_csv('cdf_error_decomp_multiscen_v13_by_pair.csv')
        ...
        if RUN_TRAINING:
            finalize_v13_checkpoint(net,norm,cached_stats,external_stats)
    else:
    ...
    write_visualization_manifest_v13()
