...
DATASET_CACHE_DIR = "dataset_cache_no_pcc_branch_limit"
DATASET_CACHE_TAG = "flex33_data800_800scen_30mc_12theta_seed0_no_pcc_branch_limit_v1"
...
RUN_BRANCH_LIMIT_SANITY = True
RUN_ALL_THETA_TRAIN = True
RUN_ALL_THETA_EVAL = True
RUN_FLEX_SYNTHESIS = False
SMOKE_TEST = True
DATASET_VERSION = "flex_support_dataset_v1"
SAVE_TRAINING_RESULT = True
TRAINING_RESULT_DIR = "training_results_theta_independent_v7style_no_pcc_branch_limit"
RUN_TRAINING = True
RUN_EVAL_ONLY = False
...
def build_ieee33_case():
    ...
    gc=GridCase(nb,0,fb,tb,r,x,0.95,1.05,fmax_p,fmax_q,pd,qd,pv_b,pv_max,0.98,gen_b,pg_min,pg_max,qg_min,qg_max,c,pb,pbu,to,rt,inb,outb)
    gc.pcc_branch_mask=pcc_branch_mask; gc.internal_branch_mask=internal_branch_mask; gc.pcc_branch_indices=pcc_branch_indices; gc.line_limit_mode=LINE_LIMIT_MODE; gc.use_explicit_pcc_limits=USE_EXPLICIT_PCC_LIMITS; gc.pcc_p_min=PCC_P_MIN; gc.pcc_p_max=PCC_P_MAX; gc.pcc_q_min=PCC_Q_MIN; gc.pcc_q_max=PCC_Q_MAX
    return gc
...
ALL_THETA_RESULT_DIR = "training_results_theta_independent_v7style_no_pcc_branch_limit_alltheta"
ALL_THETA_COMPARISON_DIR = "training_results_theta_independent_v7style_no_pcc_branch_limit_alltheta_comparison"
...
def main_v7_no_pcc_branch_limit_rebuild_dataset_alltheta():
    case=build_ieee33_case(); print_branch_limit_policy(case)
    if 'no_pcc_branch_limit' not in DATASET_CACHE_TAG: print('[warning] cache tag missing no_pcc_branch_limit',flush=True)
    if 'no_pcc_branch_limit' not in TRAINING_RESULT_DIR: print('[warning] result dir missing no_pcc_branch_limit',flush=True)
    Path(DATASET_CACHE_DIR).mkdir(parents=True,exist_ok=True)
    if BUILD_NEW_DATASET:
        XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG,active_records,all_names=get_or_build_flex_dataset_cache(case)
        policy={'line_limit_mode':case.line_limit_mode,'use_explicit_pcc_limits':case.use_explicit_pcc_limits,'pcc_branch_indices':case.pcc_branch_indices.tolist(),'pcc_p_min':case.pcc_p_min,'pcc_p_max':case.pcc_p_max,'pcc_q_min':case.pcc_q_min,'pcc_q_max':case.pcc_q_max,'internal_fmax_p':INTERNAL_FMAX_P,'internal_fmax_q':INTERNAL_FMAX_Q}
        json.dump(policy,open('branch_limit_policy.json','w'),indent=2); open('branch_limit_policy.txt','w').write(str(policy))
        json.dump({**json.load(open(DATASET_CACHE_META)),'branch_limit_policy':policy},open(DATASET_CACHE_META,'w'),indent=2)
        print('[dataset-new-policy]',flush=True); print(f'DATASET_CACHE_TAG = {DATASET_CACHE_TAG}',flush=True); print(f'line_limit_mode = {case.line_limit_mode}',flush=True); print(f'pcc_branch_indices = {case.pcc_branch_indices.tolist()}',flush=True); print(f'USE_EXPLICIT_PCC_LIMITS = {case.use_explicit_pcc_limits}',flush=True)
    else:
        XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG,active_records,all_names=get_or_build_flex_dataset_cache(case)
    if RUN_BRANCH_LIMIT_SANITY: sanity_check_theta3_no_pcc_cap(case)
    if RUN_ALL_THETA_TRAIN:
        model_map=train_or_load_all_theta_v7style_models(case,(XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG,active_records,all_names))
    else:
        model_map=load_existing_all_theta_models(case)
    # prevent old model usage
    for j,m in model_map.items():
        if isinstance(m,dict) and m.get('model_path','') and ('training_results_theta_independent_v7style' in m['model_path'] and 'no_pcc_branch_limit' not in m['model_path']):
            raise RuntimeError('Attempting to load old model path')
    if RUN_ALL_THETA_EVAL:
        eval_all_theta_multiseed_v7style(case,(XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG,active_records,all_names),model_map)
        plot_all_theta_evaluation_results()
    if RUN_FLEX_SYNTHESIS:
        for sd in FLEX_SYNTHESIS_SEEDS: synthesize_flex_domain_for_seed(case,model_map,sd,FLEX_SYNTHESIS_MC)
    compare_old_vs_new_theta3_distribution(case)

if __name__ == "__main__":
    main_v7_no_pcc_branch_limit_rebuild_dataset_alltheta()
