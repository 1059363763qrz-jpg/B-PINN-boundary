...
def main():
    ...
    if Path('all_theta_eval_cache.npz').exists():
        ...
        for j in range(THETA_FEAT.shape[0]):
            key=f'YH_theta_{j}'
            if key not in d:
                print(f"[cdf-error-decomp] warning: missing {key} in eval cache.")
                missing=True; break
            arr=np.asarray(d[key],dtype=float)
            if np.sum(np.isfinite(arr))<30:
                print(f"[cdf-error-decomp] warning: {key} valid samples < 30.")
                missing=True; break
            valid.append(arr[np.isfinite(arr)])
        if missing:
            print('[cdf-error-decomp] warning: eval cache incomplete, fallback to training YH.')
            XMU_eval=XMU
            YH_eval=YH
        else:
            m_min=min(len(v) for v in valid)
            YH_eval=np.zeros((1,m_min,THETA_FEAT.shape[0]),dtype=float)
            for j in range(THETA_FEAT.shape[0]):
                YH_eval[0,:,j]=valid[j][:m_min]
            XMU_eval=xmu_eval
    else:
