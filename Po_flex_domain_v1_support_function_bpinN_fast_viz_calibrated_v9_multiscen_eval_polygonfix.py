...
def eval_all_theta_cdf_arms_multiscenario(case,net,norm,theta_list,n_eval_scenarios=10,mc_eval_per_scenario=400,seed=SEED_EVAL,cache_path="all_theta_eval_cache_multiscen_v9.npz",rebuild_cache=True,save_prefix="all_theta_multiscen_v9"):
    if (not rebuild_cache) and Path(cache_path).exists():
        ...
        XMU_eval=np.asarray(XMU_eval,dtype=float); np.savez(cache_path,XMU_eval=XMU_eval,YH_eval=YH_eval,theta_list=np.asarray(theta_list),THETA_FEAT=np.stack([np.cos(theta_list),np.sin(theta_list)],axis=1))
    S,M,T=YH_eval.shape; rows=[]; arms_mat=np.full((S,T),np.nan)
    for s in range(S):
        for j,th in enumerate(theta_list):
            ...
            rows.append([s,j,float(th),arms,ks,q05_bp-q05_mc,q50_bp-q50_mc,q95_bp-q95_mc,(q95_bp-q05_bp)-(q95_mc-q05_mc),mean_bp-mean_mc,std_bp-std_mc,std_bp/(std_mc+1e-9)])
    ...
    arr=np.array([r[3] for r in rows]); print(f"[formal-multiscen-arms] S={S} M={M} T={T} overall mean={np.nanmean(arr):.4f} overall median={np.nanmedian(arr):.4f} overall max={np.nanmax(arr):.4f} overall q90={np.nanquantile(arr,0.9):.4f}")
    return {"XMU_eval":XMU_eval,"YH_eval":YH_eval,"arms_matrix":arms_mat}
