...
TARGETED_DIAG_DIR = "diagnostics_no_pcc_data1000_mc60_targeted"
...
def main_v7_3_targeted_diagnostics():
    print("[v7.3-targeted-diagnostics] start",flush=True)
    apply_v7_1_runtime_config()
    assert BUILD_NEW_DATASET is False
    assert DATASET_CACHE_MODE == 'load_only'
    assert RUN_ALL_THETA_TRAIN is False and RUN_ALL_THETA_EVAL is False and RUN_FLEX_SYNTHESIS is False
    Path(TARGETED_DIAG_DIR).mkdir(parents=True,exist_ok=True)
    case=build_ieee33_case()
    data_dict=get_or_build_flex_dataset_cache(case)
    global ALL_THETA_LIST
    ALL_THETA_LIST=sorted({int(t) for t,_ in DIAG_TARGET_PAIRS})
    model_map=load_existing_all_theta_models(case)
    missing=[j for j in ALL_THETA_LIST if model_map.get(j,{}).get('net') is None]
    if missing:
        raise FileNotFoundError(f"Missing data1000_mc60 trained theta models for {missing} in {ALL_THETA_RESULT_DIR}")
    targeted_rows=[]; nearest_rows=[]
    for theta_idx,seed in DIAG_TARGET_PAIRS:
        print(f"[targeted-diagnostics] theta={theta_idx:02d}, seed={seed:03d}, mc={DIAG_MC_EVAL}",flush=True)
        row,ref=run_targeted_mc2500_case(case,data_dict,model_map,int(theta_idx),int(seed)); targeted_rows.append(row)
        nearest_rows.extend(summarize_nearest_training_coverage(case,data_dict,int(theta_idx),int(seed),ref,model_map,k_list=DIAG_K_LIST))
    pd.DataFrame(targeted_rows).to_csv(f"{TARGETED_DIAG_DIR}/targeted_mc2500_metrics_by_pair.csv",index=False)
    pd.DataFrame(nearest_rows).to_csv(f"{TARGETED_DIAG_DIR}/nearest_neighbor_summary_by_pair_k.csv",index=False)
    compare_targeted_mc2500_with_existing_mc800(targeted_rows)
    print("[v7.3-targeted-diagnostics] done",flush=True)

if __name__ == "__main__":
    main_v7_3_targeted_diagnostics()
