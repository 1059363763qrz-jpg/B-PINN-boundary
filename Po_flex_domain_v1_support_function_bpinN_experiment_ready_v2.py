...
def compute_h_quantile_loss(net,XMU_scen,THETA_FEAT,YH_scen,x_mu_mean,x_mu_std,h_mean,h_std,taus=H_QUANTILE_TAUS,n_scenarios_sample=H_QUANTILE_BATCH_SCENARIOS,n_thetas_sample=H_QUANTILE_BATCH_THETAS,sample=True,rng=None):
    ...
    for i in scen_idx:
        ...
        for j in theta_idx:
            ...
            with torch.set_grad_enabled(sample):
                ...
                losses.append(((q_pred-torch.tensor(q_emp_norm,dtype=torch.float32,device=DEVICE))**2).mean())
    ...
    for ep in range(EPOCHS):
        ...
        for b in range(nb):
            ...
            if USE_H_QUANTILE_LOSS:
                ...
                hq = compute_h_quantile_loss(net,XMU_tr,THETA_FEAT,YH_tr,xmu_mean,xmu_std,h_mean,h_std,taus=H_QUANTILE_TAUS,n_scenarios_sample=H_QUANTILE_BATCH_SCENARIOS,n_thetas_sample=H_QUANTILE_BATCH_THETAS,sample=True,rng=hq_rng)
        ...
        if ep==0 or (ep+1)%50==0 or ep+1==EPOCHS:
            with torch.no_grad():
                ...
                phv,parts=physics_loss_flex(case,xr_raw_v,p0h,q0h,pgh,qgh,return_parts=True,p_scale=p0s,q_scale=q0s); h_hat_v=th_tv[:,0:1]*p0h+th_tv[:,1:2]*q0h; scv=(((h_hat_v-yh_tv)/(hs+1e-9))**2).mean(); val_hq_rng=np.random.default_rng(SEED_TRAIN + 999); hqv=compute_h_quantile_loss(net,XMU_va,THETA_FEAT,YH_va,xmu_mean,xmu_std,h_mean,h_std,taus=H_QUANTILE_TAUS,n_scenarios_sample=min(H_QUANTILE_BATCH_SCENARIOS,max(1,XMU_va.shape[0])),n_thetas_sample=H_QUANTILE_BATCH_THETAS,sample=False,rng=val_hq_rng)
