...
ALL_THETA_COMPARISON_DIR = "training_results_theta_independent_v7style_no_pcc_branch_limit_data1000_mc60_qcal_only_comparison"
...
def main_v7_4_quantile_calibration_only():
    print('[v7.4.1-qcal-alltheta-skip-existing] start',flush=True)
    apply_v7_1_runtime_config()
    global ALL_THETA_LIST, TRAIN_THETA_LIST, HARD_THETA_LIST
    HARD_THETA_LIST=list(range(N_THETA)); TRAIN_THETA_LIST=list(range(N_THETA)); ALL_THETA_LIST=list(range(N_THETA))
    ...
    Path(ALL_THETA_COMPARISON_DIR).mkdir(parents=True,exist_ok=True)
    case=build_ieee33_case(); data_dict=get_or_build_flex_dataset_cache(case)
    print('old_loss = nll + LAM_T_SUP*t_sup + LAM_DISPATCH_SUP*disp + LAM_PHYS_FLEX*phys + beta_kl*kl',flush=True)
    print(f'qcal taus = {SCENARIO_Q_TAUS}, weights = {SCENARIO_Q_WEIGHTS}',flush=True)
    skipped=[]; trained=[]
    for theta_idx in ALL_THETA_LIST:
        model_path=Path(f"{ALL_THETA_RESULT_DIR}/theta_independent_theta{theta_idx:02d}_v7style_model.pt")
        norm_path=Path(f"{ALL_THETA_RESULT_DIR}/theta_independent_theta{theta_idx:02d}_v7style_norm.pkl")
        if SKIP_QCAL_TRAIN_IF_MODEL_EXISTS and model_path.exists() and norm_path.exists():
            print(f"[v7.4-qcal-only] theta={theta_idx:02d} existing qcal model found, skip training",flush=True)
            skipped.append(theta_idx); continue
        print(f'[v7.4-qcal-only] train theta={theta_idx:02d}',flush=True)
        train_single_theta_v7style_model_with_qcal(case,data_dict,theta_idx); trained.append(theta_idx)
    missing_after=[]
    for theta_idx in ALL_THETA_LIST:
        if not (Path(f"{ALL_THETA_RESULT_DIR}/theta_independent_theta{theta_idx:02d}_v7style_model.pt").exists() and Path(f"{ALL_THETA_RESULT_DIR}/theta_independent_theta{theta_idx:02d}_v7style_norm.pkl").exists()):
            missing_after.append(theta_idx)
    print(f"[v7.4-qcal-only] skipped theta list = {skipped}",flush=True)
    print(f"[v7.4-qcal-only] newly trained theta list = {trained}",flush=True)
    print(f"[v7.4-qcal-only] missing theta list = {missing_after}",flush=True)
    ...
    print(f"[v7.4-qcal-only] targeted MC2500 output dir = {TARGETED_DIAG_DIR}",flush=True)
    run_qcal_only_targeted_mc2500_eval(case,data_dict)
    print('[v7.4.1-qcal-alltheta-skip-existing] done',flush=True)

if __name__ == '__main__':
    main_v7_4_quantile_calibration_only()
