...
TRAINING_RUN_TAG = "v14_rollback_v12_strategy_keep_visuals"
...
def eval_and_plot_flex_domain_posterior(case,net,norm,theta_list,n_post=30,mc_eval=200):
    ...
    plt.title('B-PINN posterior uncertainty of median flexibility domain'); plt.xlabel('P0 (MW)'); plt.ylabel('Q0 (Mvar)'); plt.grid(alpha=0.25); plt.legend(); plt.axis('equal'); plt.tight_layout(); plt.savefig('FlexDomain_posterior_overlay_v14.png',dpi=280)
...
def eval_and_plot_realization_cloud(case,net,norm,theta_list,n_real=24):
    ...
    plt.title('Realization cloud of flexibility domains under source-load uncertainty'); plt.xlabel('P0 (MW)'); plt.ylabel('Q0 (Mvar)'); plt.grid(alpha=0.25); plt.legend(['MC realization domains','MC median domain','B-PINN median domain']); plt.axis('equal'); plt.tight_layout(); plt.savefig('FlexDomain_realization_cloud_v14.png',dpi=280)
...
def compare_v14_against_v12_v13(v14_pair_csv='cdf_error_decomp_multiscen_v14_by_pair.csv'):
    ...
    m.to_csv('v14_vs_v12_v13_multiscen_comparison.csv',index=False)
...
def write_visualization_manifest_v14():
    rows=[
...
['FlexDomain_posterior_overlay_v14.png','flexdomain','eval_and_plot_flex_domain_posterior','后验叠加','if enabled','v14'],
['FlexDomain_realization_cloud_v14.png','flexdomain','eval_and_plot_realization_cloud','实现云图','if enabled','v14'],
...
['v14_vs_v12_v13_multiscen_comparison.csv','comparison','compare_v14_against_v12_v13','v14对比v12/v13外部多场景对比','after formal cache eval','v14']
]
    import csv
    with open('visualization_manifest_v14.csv','w',newline='',encoding='utf-8') as f:
        w=csv.writer(f); w.writerow(['filename','category','function_name','purpose','generated_when','version_status']); w.writerows(rows)
    print('[visualization-manifest-v14] saved visualization_manifest_v14.csv')
...
def main():
    ...
    write_visualization_manifest_v14()
