...
TRAINING_RUN_TAG = "expB_v7_extendedCD_best_seed0"
...
def eval_all_theta_cdf_arms_multiscenario(case,net,norm,theta_list,n_eval_scenarios=10,mc_eval_per_scenario=400,seed=SEED_EVAL,cache_path="all_theta_eval_cache_multiscen_v9.npz",rebuild_cache=True,save_prefix="all_theta_multiscen_v9"):
    formal_active_rebuilt = False
    if (not rebuild_cache) and Path(cache_path).exists():
        ...
        for s in range(S):
            ...
            for m in range(mc_eval_per_scenario):
                ...
                for j,th in enumerate(theta_list):
                    v=solve_flex_support_gurobi_33bus(case,pd,qd,pr,qr,np.cos(th),np.sin(th)); YH_eval[s,m,j]=v if np.isfinite(v) else np.nan
        XMU_eval=np.asarray(XMU_eval,dtype=float)
    S,M,T=YH_eval.shape; rows=[]; arms_mat=np.full((S,T),np.nan); ks_mat=np.full((S,T),np.nan); q05_mat=np.full((S,T),np.nan); q50_mat=np.full((S,T),np.nan); q95_mat=np.full((S,T),np.nan); span_mat=np.full((S,T),np.nan); mean_mat=np.full((S,T),np.nan); std_mat=np.full((S,T),np.nan); stdr_mat=np.full((S,T),np.nan)
    for s in range(S):
        for j,th in enumerate(theta_list):
            ys=YH_eval[s,:,j]; ys=ys[np.isfinite(ys)]
            if len(ys)<30: continue
            ys=np.sort(ys); z=np.linspace(ys.min()-0.2,ys.max()+0.2,250); cdf_mc=np.searchsorted(ys,z,side='right')/len(ys)
            xt=torch.as_tensor((XMU_eval[s:s+1]-norm['x_mu_mean'])/norm['x_mu_std'],dtype=torch.float32,device=DEVICE); th_t=torch.as_tensor([[np.cos(th),np.sin(th)]],dtype=torch.float32,device=DEVICE)
            with torch.no_grad(): _,w,mu,sg=net.forward_gmm(xt,th_t,sample=False)
            wp,mp,sp=w.cpu().numpy().reshape(-1),mu.cpu().numpy().reshape(-1),sg.cpu().numpy().reshape(-1)
            cdf_bp=gmm_cdf((z-float(norm['h_mean'][0,0]))/(float(norm['h_std'][0,0])+1e-9),wp,mp,sp)
            arms=100*math.sqrt(np.mean((cdf_bp-cdf_mc)**2)); ks=float(np.max(np.abs(cdf_bp-cdf_mc))); arms_mat[s,j]=arms
            q05_bp=gmm_quantile([0.05],wp,mp,sp)[0]*float(norm['h_std'][0,0])+float(norm['h_mean'][0,0]); q50_bp=gmm_quantile([0.5],wp,mp,sp)[0]*float(norm['h_std'][0,0])+float(norm['h_mean'][0,0]); q95_bp=gmm_quantile([0.95],wp,mp,sp)[0]*float(norm['h_std'][0,0])+float(norm['h_mean'][0,0])
            q05_mc,q50_mc,q95_mc=np.quantile(ys,0.05),np.quantile(ys,0.5),np.quantile(ys,0.95); mean_mc,std_mc=float(np.mean(ys)),float(np.std(ys))
            mu_mix=np.sum(wp*mp); var_mix=np.sum(wp*(sp*sp+mp*mp))-mu_mix*mu_mix; mean_bp=mu_mix*float(norm['h_std'][0,0])+float(norm['h_mean'][0,0]); std_bp=np.sqrt(max(var_mix,1e-12))*float(norm['h_std'][0,0])
            q05e=q05_bp-q05_mc; q50e=q50_bp-q50_mc; q95e=q95_bp-q95_mc; spane=(q95_bp-q05_bp)-(q95_mc-q05_mc); meane=mean_bp-mean_mc; stde=std_bp-std_mc; stdre=std_bp/(std_mc+1e-9)
            ks_mat[s,j]=ks; q05_mat[s,j]=q05e; q50_mat[s,j]=q50e; q95_mat[s,j]=q95e; span_mat[s,j]=spane; mean_mat[s,j]=meane; std_mat[s,j]=stde; stdr_mat[s,j]=stdre
            rows.append([s,j,float(th),arms,ks,q05e,q50e,q95e,spane,meane,stde,stdre])
    with open(f"{save_prefix}_by_scenario_theta.csv","w",newline="",encoding="utf-8") as f:
        w=csv.writer(f); w.writerow(["scenario_idx","theta_idx","theta","arms_pct","ks_stat","q05_err","q50_err","q95_err","span95_err","mean_err","std_err","std_ratio"]); w.writerows(rows)
    np.savez(cache_path,XMU_eval=XMU_eval,YH_eval=YH_eval,theta_list=np.asarray(theta_list),THETA_FEAT=np.stack([np.cos(theta_list),np.sin(theta_list)],axis=1),arms_matrix=arms_mat,ks_matrix=ks_mat,q05_err_matrix=q05_mat,q50_err_matrix=q50_mat,q95_err_matrix=q95_mat,span95_err_matrix=span_mat,mean_err_matrix=mean_mat,std_err_matrix=std_mat,std_ratio_matrix=stdr_mat)
    th_rows=[]; sc_rows=[]
    for j,th in enumerate(theta_list): v=arms_mat[:,j]; th_rows.append([j,float(th),float(np.nanmean(v)),float(np.nanmedian(v)),float(np.nanmax(v)),float(np.nanquantile(v,0.9)),float(np.nanmean(q50_mat[:,j])),float(np.nanmean(span_mat[:,j])),float(np.nanmean(stdr_mat[:,j]))])
    for s in range(S): v=arms_mat[s,:]; sc_rows.append([s,float(np.nanmean(v)),float(np.nanmedian(v)),float(np.nanmax(v)),float(np.nanquantile(v,0.9)),int(np.nanargmax(v)) if np.isfinite(v).any() else -1])
    with open(f"{save_prefix}_by_theta_summary.csv","w",newline="",encoding="utf-8") as f: w=csv.writer(f); w.writerow(["theta_idx","theta","mean_arms","median_arms","max_arms","q90_arms","mean_q50_err","mean_span95_err","mean_std_ratio"]); w.writerows(th_rows)
    with open(f"{save_prefix}_by_scenario_summary.csv","w",newline="",encoding="utf-8") as f: w=csv.writer(f); w.writerow(["scenario_idx","mean_arms","median_arms","max_arms","q90_arms","worst_theta_idx"]); w.writerows(sc_rows)
    arr=np.array([r[3] for r in rows]); wi=np.unravel_index(np.nanargmax(arms_mat),arms_mat.shape)
    print(f"[formal-multiscen-arms] S={S} M={M} T={T} overall mean={np.nanmean(arr):.4f} overall median={np.nanmedian(arr):.4f} overall max={np.nanmax(arr):.4f} overall q90={np.nanquantile(arr,0.9):.4f} worst scenario={wi[0]} worst theta={wi[1]} worst pair=({wi[0]}, {wi[1]}, {arms_mat[wi]:.4f})")
    plt.figure(figsize=(9,4),dpi=180); plt.bar(np.arange(T),np.nanmean(arms_mat,axis=0)); plt.tight_layout(); plt.savefig(f"{save_prefix}_theta_bar.png"); plt.close()
    plt.figure(figsize=(9,4),dpi=180); plt.bar(np.arange(S),np.nanmean(arms_mat,axis=1)); plt.tight_layout(); plt.savefig(f"{save_prefix}_scenario_bar.png"); plt.close()
    plt.figure(figsize=(8,5),dpi=180); plt.imshow(arms_mat,aspect='auto',cmap='magma'); plt.colorbar(label='ARMS (%)'); plt.tight_layout(); plt.savefig(f"{save_prefix}_heatmap.png"); plt.close()
    return {"XMU_eval":XMU_eval,"YH_eval":YH_eval,"arms_matrix":arms_mat,"q50_err_matrix":q50_mat,"span95_err_matrix":span_mat,"std_ratio_matrix":stdr_mat}
...
def analyze_cdf_error_decomposition_multiscenario(net,norm,XMU_eval,THETA_FEAT,theta_list,YH_eval,save_prefix="cdf_error_decomp_multiscen_v10",cdf_grid_size=300,n_posterior_samples=1):
    rows=[]
    for sidx in range(YH_eval.shape[0]):
        for j,th in enumerate(theta_list):
            h_mc=np.asarray(YH_eval[sidx,:,j],dtype=float); h_mc=h_mc[np.isfinite(h_mc)]
            if len(h_mc)<30: continue
            hmean=float(h_mc.mean()); hstd=float(h_mc.std()+1e-9); hr=max(1e-9,h_mc.max()-h_mc.min())
            z=np.linspace(h_mc.min()-0.05*hr,h_mc.max()+0.05*hr,cdf_grid_size); F_mc=np.searchsorted(np.sort(h_mc),z,side='right')/len(h_mc)
            xmu=XMU_eval[sidx:sidx+1]; xt=torch.tensor((xmu-norm['x_mu_mean'])/norm['x_mu_std'],dtype=torch.float32,device=DEVICE); th_t=torch.tensor(THETA_FEAT[j:j+1],dtype=torch.float32,device=DEVICE)
            cdfs=[]
            for _ in range(max(1,n_posterior_samples)):
                with torch.no_grad(): _,w,mu,s=net.forward_gmm(xt,th_t,sample=True)
                zn=(z-hmean)/(hstd); cdfs.append(gmm_cdf(zn,w.cpu().numpy().reshape(-1),mu.cpu().numpy().reshape(-1),s.cpu().numpy().reshape(-1)))
            ...
            qmc=np.quantile(h_mc,[0.05,0.5,0.95]);
            with torch.no_grad(): _,w0,mu0,s0=net.forward_gmm(xt,th_t,sample=False); qn=gmm_quantile_torch(w0,mu0,s0,[0.05,0.5,0.95]).cpu().numpy().reshape(-1)
            qbp=hmean+hstd*qn; q05,q50,q95=(qbp-qmc).tolist(); span95=(qbp[2]-qbp[0])-(qmc[2]-qmc[0])
            bp_mean=float(hmean+hstd*(w0*mu0).sum(dim=1).cpu().numpy()[0]); bp_var=float((w0*(s0*s0+mu0*mu0)).sum(dim=1).cpu().numpy()[0]-(w0*mu0).sum(dim=1).cpu().numpy()[0]**2)
            mc_mean=float(h_mc.mean()); mc_std=float(h_mc.std()); bp_std=float(abs(hstd)*np.sqrt(max(0,bp_var))); std_ratio=float(bp_std/(mc_std+1e-9))
            q10,q90=np.quantile(h_mc,[0.1,0.9]); m1=z<=q10; m2=(z>q10)&(z<q90); m3=z>=q90
            lo=100*np.sqrt(np.mean((F_bp[m1]-F_mc[m1])**2)) if m1.any() else np.nan; mi=100*np.sqrt(np.mean((F_bp[m2]-F_mc[m2])**2)) if m2.any() else np.nan; up=100*np.sqrt(np.mean((F_bp[m3]-F_mc[m3])**2)) if m3.any() else np.nan
            tags=[]
            if abs(q50)>0.1*hr: tags.append('location_shift_error')
            if abs(std_ratio-1)>0.25 or abs(span95)>0.15*hr: tags.append('scale_mismatch_error')
            if arms>=8 and not tags: tags.append('shape_mismatch_error')
            if not tags: tags=['well_calibrated']
            rows.append(dict(scenario_idx=sidx,theta_idx=j,theta=float(th),arms_pct=float(arms),ks_stat=float(ks),signed_cdf_bias=float(sb),abs_cdf_bias=float(ab),lower_cdf_arms=float(lo),middle_cdf_arms=float(mi),upper_cdf_arms=float(up),q05_err=float(q05),q50_err=float(q50),q95_err=float(q95),span95_err=float(span95),mean_err=float(bp_mean-mc_mean),std_err=float(bp_std-mc_std),std_ratio=float(std_ratio),primary_error_type=tags[0],error_type_multi=';'.join(tags)))
    import pandas as pd
    df=pd.DataFrame(rows); df.to_csv(f'{save_prefix}_by_pair.csv',index=False)
    by_theta=df.groupby(['theta_idx','theta'],as_index=False).agg({'arms_pct':['mean','median','max',lambda x:np.quantile(x,0.9)],'q50_err':'mean','span95_err':'mean','std_ratio':'mean'})
    by_theta.columns=['theta_idx','theta','mean_arms','median_arms','max_arms','q90_arms','mean_q50_err','mean_span95_err','mean_std_ratio']; by_theta.to_csv(f'{save_prefix}_by_theta.csv',index=False)
    by_sc=df.groupby('scenario_idx',as_index=False).agg(mean_arms=('arms_pct','mean'),max_arms=('arms_pct','max'),q90_arms=('arms_pct',lambda x:np.quantile(x,0.9)),worst_theta_idx=('arms_pct',lambda x:int(df.loc[x.index[np.argmax(x.values)],'theta_idx']))); by_sc.to_csv(f'{save_prefix}_by_scenario.csv',index=False)
    w=df.iloc[df['arms_pct'].idxmax()]
    summ=pd.DataFrame([{'overall_mean_arms':df['arms_pct'].mean(),'overall_median_arms':df['arms_pct'].median(),'overall_max_arms':df['arms_pct'].max(),'overall_q90_arms':df['arms_pct'].quantile(0.9),'num_pairs_over_10pct':int((df['arms_pct']>10).sum()),'num_pairs_over_15pct':int((df['arms_pct']>15).sum()),'num_pairs_over_20pct':int((df['arms_pct']>20).sum()),'num_pairs_over_30pct':int((df['arms_pct']>30).sum()),'num_pairs_over_40pct':int((df['arms_pct']>40).sum()),'worst_scenario':int(w.scenario_idx),'worst_theta':int(w.theta_idx),'worst_pair_arms':float(w.arms_pct)}]); summ.to_csv(f'{save_prefix}_summary.csv',index=False)
...
def diagnose_scenario_coverage_shift(XMU_train,XMU_eval,scenario_summary_csv,norm,save_path='scenario_generalization_diagnostics_v10.csv'):
    import pandas as pd
    tr=(XMU_train-norm['x_mu_mean'])/norm['x_mu_std']; ev=(XMU_eval-norm['x_mu_mean'])/norm['x_mu_std']
    d=np.linalg.norm(ev[:,None,:]-tr[None,:,:],axis=2); nd=d.min(axis=1); center=np.linalg.norm(ev-tr.mean(axis=0,keepdims=True),axis=1)
    pct=np.array([100*np.mean(nd<=x) for x in nd]); n1=(d<=1.0).sum(axis=1); n2=(d<=2.0).sum(axis=1)
    sc=pd.read_csv(scenario_summary_csv)
    out=sc[['scenario_idx','mean_arms','max_arms','q90_arms','worst_theta_idx']].copy(); out['nearest_train_dist']=nd; out['center_dist']=center; out['nearest_dist_percentile']=pct; out['num_train_neighbors_r1']=n1; out['num_train_neighbors_r2']=n2; out.to_csv(save_path,index=False)
...
def generate_worst_theta_diagnostics(pair_csv,theta_csv,save_path='worst_theta_diagnostics_v10.csv'):
    import pandas as pd
    ...
    out=th.merge(dom,on='theta_idx',how='left')
    for c,t in [(10,'num_scenarios_over_10pct'),(15,'num_scenarios_over_15pct'),(20,'num_scenarios_over_20pct')]: out[t]=out.theta_idx.map(df.groupby('theta_idx').apply(lambda g:int((g.arms_pct>c).sum())))
    def act(r): return 'scenario-conditioned bias / active-region-conditioned mean correction' if r.dominant_error_type=='location_shift_error' else ('scenario-conditioned scale calibration / CRPS-like scale loss' if r.dominant_error_type=='scale_mismatch_error' else 'theta-specific GMM head or active-pattern-conditioned mixture')
    out['suggested_action']=out.apply(act,axis=1); out.to_csv(save_path,index=False)
...
def plot_worst_pair_cdfs(pair_csv,net,norm,XMU_eval,THETA_FEAT,theta_list,YH_eval):
    ...
    def one(ax,row):
        sidx=int(row.scenario_idx); j=int(row.theta_idx); h_mc=np.asarray(YH_eval[sidx,:,j],dtype=float); hmean=h_mc.mean(); hstd=h_mc.std()+1e-9; hr=max(1e-9,h_mc.max()-h_mc.min()); z=np.linspace(h_mc.min()-0.05*hr,h_mc.max()+0.05*hr,300); F_mc=np.searchsorted(np.sort(h_mc),z,side='right')/len(h_mc)
        xt=torch.tensor((XMU_eval[sidx:sidx+1]-norm['x_mu_mean'])/norm['x_mu_std'],dtype=torch.float32,device=DEVICE); th_t=torch.tensor(THETA_FEAT[j:j+1],dtype=torch.float32,device=DEVICE)
        with torch.no_grad(): _,w,mu,s=net.forward_gmm(xt,th_t,sample=False)
        F_bp=gmm_cdf((z-hmean)/hstd,w.cpu().numpy().reshape(-1),mu.cpu().numpy().reshape(-1),s.cpu().numpy().reshape(-1)); ax.plot(z,F_mc,'k',label='MC'); ax.plot(z,F_bp,'r--',label='B-PINN'); ax.set_title(f"sc={sidx} th={j} ARMS={row.arms_pct:.2f} q50={row.q50_err:.3f} span95={row.span95_err:.3f} stdR={row.std_ratio:.2f}")
...
def main():
    ...
    if RUN_TRAINING:
        ...
        if RUN_EVAL_ONLY:
            cv=eval_cached_validation_cdf_arms(net,norm,XMU,THETA_FEAT,THETA_LIST,YH,save_prefix="cached_val_cdf_arms_evalonly_v9")
    ...
    if RUN_FULL_OPF_EVAL:
        ...
        if MULTI_SCENARIO_FORMAL_EVAL:
            eval_pack=eval_all_theta_cdf_arms_multiscenario(case,net,norm,THETA_LIST,n_eval_scenarios=N_FORMAL_EVAL_SCENARIOS,mc_eval_per_scenario=MC_EVAL_PER_SCENARIO,cache_path=FORMAL_EVAL_CACHE_PATH,rebuild_cache=FORMAL_EVAL_REBUILD_CACHE,save_prefix="all_theta_multiscen_v9")
    ...
    if MULTI_SCENARIO_FORMAL_EVAL and Path(FORMAL_EVAL_CACHE_PATH).exists():
        ...
        pair_df,_,_,_=analyze_cdf_error_decomposition_multiscenario(net,norm,XMU_eval,THETA_FEAT,THETA_LIST,YH_eval,save_prefix='cdf_error_decomp_multiscen_v10',cdf_grid_size=300,n_posterior_samples=1)
        diagnose_scenario_coverage_shift(XMU,XMU_eval,'cdf_error_decomp_multiscen_v10_by_scenario.csv',norm,save_path='scenario_generalization_diagnostics_v10.csv')
        generate_worst_theta_diagnostics('cdf_error_decomp_multiscen_v10_by_pair.csv','cdf_error_decomp_multiscen_v10_by_theta.csv',save_path='worst_theta_diagnostics_v10.csv')
        plot_worst_pair_cdfs('cdf_error_decomp_multiscen_v10_by_pair.csv',net,norm,XMU_eval,THETA_FEAT,THETA_LIST,YH_eval)
