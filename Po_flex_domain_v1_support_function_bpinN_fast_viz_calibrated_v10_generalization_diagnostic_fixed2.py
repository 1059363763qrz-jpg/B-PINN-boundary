...
def analyze_cdf_error_decomposition_multiscenario(net,norm,XMU_eval,THETA_FEAT,theta_list,YH_eval,save_prefix="cdf_error_decomp_multiscen_v10",cdf_grid_size=300,n_posterior_samples=1):
    ...
    for sidx in range(YH_eval.shape[0]):
        for j,th in enumerate(theta_list):
            ...
            z=np.linspace(h_mc.min()-0.05*hr,h_mc.max()+0.05*hr,cdf_grid_size)
            F_mc=np.searchsorted(np.sort(h_mc),z,side='right')/len(h_mc)
            xmu=XMU_eval[sidx:sidx+1]
            xt=torch.tensor((xmu-norm['x_mu_mean'])/norm['x_mu_std'],dtype=torch.float32,device=DEVICE)
            ...
            for _ in range(max(1,n_posterior_samples)):
                with torch.no_grad(): _,w,mu,sig=net.forward_gmm(xt,th_t,sample=True)
                zn=(z-hm)/(hs+1e-9)
                cdfs.append(gmm_cdf(zn,w.cpu().numpy().reshape(-1),mu.cpu().numpy().reshape(-1),sig.cpu().numpy().reshape(-1)))
            ...
            qn=gmm_quantile_torch(w0,mu0,s0,[0.05,0.5,0.95]).cpu().numpy().reshape(-1)
            qmc=np.quantile(h_mc,[0.05,0.5,0.95]); qbp=hm+hs*qn
            q05,q50,q95=(qbp-qmc).tolist(); span95=(qbp[2]-qbp[0])-(qmc[2]-qmc[0])
            mu_mix=float((w0*mu0).sum(dim=1).cpu().numpy()[0])
            var_mix=float((w0*(s0*s0+mu0*mu0)).sum(dim=1).cpu().numpy()[0]-mu_mix**2)
            bp_mean=float(hm+hs*mu_mix); bp_std=float(abs(hs)*np.sqrt(max(0.0,var_mix)))
            mc_mean=float(h_mc.mean()); mc_std=float(h_mc.std()); std_ratio=float(bp_std/(mc_std+1e-9))
            q10,q90=np.quantile(h_mc,[0.1,0.9]); m1=z<=q10; m2=(z>q10)&(z<q90); m3=z>=q90
            lo=100*np.sqrt(np.mean((F_bp[m1]-F_mc[m1])**2)) if m1.any() else np.nan
            mi=100*np.sqrt(np.mean((F_bp[m2]-F_mc[m2])**2)) if m2.any() else np.nan
            up=100*np.sqrt(np.mean((F_bp[m3]-F_mc[m3])**2)) if m3.any() else np.nan
            tags=[]
...
def check_multiscen_decomp_consistency(decomp_pair_csv="cdf_error_decomp_multiscen_v10_by_pair.csv",formal_pair_csv="all_theta_multiscen_v9_by_scenario_theta.csv",tolerance_mean=1.0,tolerance_max=3.0):
    import pandas as pd
    ...
    m=d2[['scenario_idx','theta_idx','arms_pct']].rename(columns={'arms_pct':'arms_formal'}).merge(
        d1[['scenario_idx','theta_idx','arms_pct']].rename(columns={'arms_pct':'arms_decomp'}),on=['scenario_idx','theta_idx'],how='inner')
    ...
    print(f'mean abs ARMS diff = {mean_abs:.4f}')
    print(f'max abs ARMS diff = {max_abs:.4f}')
    print(f'corr = {corr:.4f}')
    if (np.isfinite(mean_abs) and mean_abs>tolerance_mean) or (np.isfinite(max_abs) and max_abs>tolerance_max):
...
def main():
    ...
    if MULTI_SCENARIO_FORMAL_EVAL and Path(FORMAL_EVAL_CACHE_PATH).exists():
        ...
        scenario_src='all_theta_multiscen_v9_by_scenario_summary.csv' if Path('all_theta_multiscen_v9_by_scenario_summary.csv').exists() else 'cdf_error_decomp_multiscen_v10_by_scenario.csv'
        diagnose_scenario_coverage_shift(XMU,XMU_eval,scenario_src,norm,save_path='scenario_generalization_diagnostics_v10.csv')
        theta_src='all_theta_multiscen_v9_by_theta_summary.csv' if Path('all_theta_multiscen_v9_by_theta_summary.csv').exists() else 'cdf_error_decomp_multiscen_v10_by_theta.csv'
        generate_worst_theta_diagnostics('cdf_error_decomp_multiscen_v10_by_pair.csv',theta_src,save_path='worst_theta_diagnostics_v10.csv')
        check_multiscen_decomp_consistency('cdf_error_decomp_multiscen_v10_by_pair.csv','all_theta_multiscen_v9_by_scenario_theta.csv')
