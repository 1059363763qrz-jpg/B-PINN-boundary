...
MC_EVAL_PER_SCENARIO = 400
FORMAL_EVAL_REBUILD_CACHE = True
...
def support_values_to_polygon(theta_list,h_values,eps=1e-9,do_convex_cleanup=True):
    ...
    if h_values.ndim!=1 or len(h_values)!=len(theta_list): return None
    if len(theta_list)<3 or (not np.all(np.isfinite(h_values))) or np.std(h_values)<POLYGON_JITTER_EPS: return None
...
def main():
    ...
    if YH_eval.ndim==3 and YH_eval.shape[0]>1:
        with open('cdf_error_decomp_multiscen_by_pair_v9.csv','w',newline='',encoding='utf-8') as f:
            w=csv.writer(f); w.writerow([f'theta_{j}' for j in range(YH_eval.shape[-1])]); [w.writerow(r.tolist()) for r in YH_eval.reshape(-1,YH_eval.shape[-1])]
