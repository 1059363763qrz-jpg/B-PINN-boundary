...
H_QUANTILE_TAUS = [0.05,0.25,0.5,0.75,0.95]
...
def train_bayes_flex_gmm2(case,XMU,XREAL,THETA_FEAT,YH,YP0,YQ0,YPG,YQG):
    ...
    for ep in range(EPOCHS):
        perm=rng.permutation(n); beta=BETA_KL_MAX*min(1.0,(ep+1)/KL_WARMUP_EPOCHS); acc=np.zeros(9)
        for b in range(nb):
            ...
            henc,w,mu1,s1,mu2,s2=net.forward_gmm(xmu,th,sample=True); yh_n=(yh-hm)/(hs+1e-9); nll=(-gmm2_log_prob(yh_n,w,mu1,s1,mu2,s2)).mean()
            ...
            if USE_H_QUANTILE_LOSS and (ep+1)>=H_QUANTILE_START_EPOCH and (b % QUANTILE_EVERY_N_BATCHES == 0):
                hq_rng=np.random.default_rng(SEED_TRAIN + 100000*ep + b)
                hq = compute_h_quantile_loss(net,XMU_tr,THETA_FEAT,QH_EMP_TR,xmu_mean,xmu_std,h_mean,h_std,taus=H_QUANTILE_TAUS,n_scenarios_sample=H_QUANTILE_BATCH_SCENARIOS,n_thetas_sample=H_QUANTILE_BATCH_THETAS,sample=True,rng=hq_rng)
            ...
            acc += np.array([float(loss.detach().cpu()),float(nll.detach().cpu()),float(bsup.detach().cpu()),float(p0q0_sup.detach().cpu()),float(t_sup.detach().cpu()),float(dsup.detach().cpu()),float(phys.detach().cpu()),float(scons.detach().cpu()),float(hq.detach().cpu())])
        if ep==0 or (ep+1)%50==0 or ep+1==EPOCHS:
            with torch.no_grad():
                henc,w,mu1,s1,mu2,s2=net.forward_gmm(xmu_tv,th_tv,sample=False); nllv=(-gmm2_log_prob((yh_tv-hm)/(hs+1e-9),w,mu1,s1,mu2,s2)).mean()
            ...
            print(f"Epoch {ep+1:4d} | avg_loss={acc[0]/nb:.6f} avg_nll_h={acc[1]/nb:.6f} avg_boundary_sup={acc[2]/nb:.6f} avg_p0q0_sup={acc[3]/nb:.6f} avg_t_sup={acc[4]/nb:.6f} avg_dispatch_sup={acc[5]/nb:.6f} avg_phys_flex={acc[6]/nb:.6f} avg_support_consist={acc[7]/nb:.6f} avg_h_quantile={acc[8]/nb:.6f} avg_kl/N={float((net.kl_divergence()/n).detach().cpu()):.6f} beta={beta:.4f} n_batches={nb} | val_loss={lv.item():.6f} val_nll_h={nllv.item():.6f} val_boundary_sup={bsv.item():.6f} val_p0q0_sup={p0q0sv.item():.6f} val_t_sup={tsv.item():.6f} val_dispatch_sup={dsv.item():.6f} val_phys_flex={phv.item():.6f} val_support_consist={scv.item():.6f} val_h_quantile={hqv.item():.6f} val_pcc_p={parts['pcc_p'].item():.3e} val_pcc_q={parts['pcc_q'].item():.3e} val_global_p={parts['global_p'].item():.3e} val_global_q={parts['global_q'].item():.3e} val_raw_pcc_p={parts['raw_pcc_p'].item():.3e} val_raw_pcc_q={parts['raw_pcc_q'].item():.3e}")
    ...
    for ax,j in zip(axs.ravel(),idx):
        ...
        with torch.no_grad():
            for _ in range(EVAL_THETA_SAMPLES):
                ...
                cdfs.append(gmm2_cdf(z_norm,w.cpu().numpy().reshape(-1),float(mu1),float(s1),float(mu2),float(s2)))
...
def eval_and_plot_flex_domain(case,net,norm,theta_list,mc_eval=500,seed=SEED_EVAL):
    ...
    for th in theta_list:
        ...
        with torch.no_grad():
            ...
            qn=gmm2_quantile_torch(w,mu1,s1,mu2,s2,[0.05,0.5,0.95]).cpu().numpy().reshape(-1)
    ...
    for s in range(N_TEST_SCENARIOS):
        ...
        for th in theta_list:
            ...
            with torch.no_grad():
                c=[]
                for _ in range(EVAL_THETA_SAMPLES):
                    _,w,mu1,s1,mu2,s2=net.forward_gmm(xt,th_t,sample=True); z_norm=(z-float(norm['h_mean'][0,0]))/(float(norm['h_std'][0,0])+1e-9); c.append(gmm2_cdf(z_norm,w.cpu().numpy().reshape(-1),float(mu1),float(s1),float(mu2),float(s2)))
