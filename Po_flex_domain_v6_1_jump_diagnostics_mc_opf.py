...
def run_mc_opf_jump_diagnostic(case,theta_idx,theta_list,seed,mc,sampler="clip"):
    pd_mu,qd_mu,pr_mu,qr_mu=draw_diag_external_scenario(case,seed); theta=float(theta_list[theta_idx]); alpha=float(np.cos(theta)); beta=float(np.sin(theta)); rows=[]; succ=0; infeas=0; t0=time.perf_counter(); np.random.seed(seed+100000+theta_idx)
    for m in range(mc):
        ...
        if sol.get('ok',False):
            succ+=1; sig_id,act=get_active_constraint_signature(case,sol)
            rec.update({'h':float(sol.get('h',np.nan)),'h_stage1':float(sol.get('h_stage1',np.nan)),'P0':float(sol.get('P0',np.nan)),'Q0':float(sol.get('Q0',np.nan)),'support_residual':float(abs(sol.get('h',np.nan)-alpha*sol.get('P0',0)-beta*sol.get('Q0',0))),
                        ...
                        'Qline_abs_max':float(np.max(np.abs(sol.get('Q',np.array([np.nan]))))),'active_signature_id':sig_id,'active_names':'|'.join(act)})
        else:
            infeas+=1
        rows.append(rec)
...
def run_sampler_comparison_for_case(case,theta_idx,seed,mc):
    clip_df=run_mc_opf_jump_diagnostic(case,theta_idx,THETA_LIST,seed,mc,sampler='clip'); csm,cat,cw=analyze_h_jump_distribution(clip_df,theta_idx,seed,mc,'clip')
...
def main_jump_diagnostics():
    ...
    for item in DIAG_CASES:
        ...
        if RUN_TRUE_TRUNC_COMPARISON: run_sampler_comparison_for_case(case,theta_idx,seed,mc)
